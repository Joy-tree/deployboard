...
app.get('/api/workspace', requireAuth, async (req, res) => {
  ...
  res.json({
    ...
    // Pass uploadedProjects through so bootstrapSession can populate the upload page on any device
...
app.post('/api/workspace', requireAuth, async (req, res) => {
  try {
    ...
    // CRITICAL: Read existing Firebase workspace first to preserve fields like
    // uploadedProjects that the frontend dashboard sync does NOT send.
    // Without this, every queueWorkspaceSync() call wipes uploadedProjects.
    let existingFbWs = {};
    try { existingFbWs = (await readWorkspaceFromFirebase(req.user)) || {}; } catch {}
    const workspace = {
      // Preserve any existing fields not managed by the dashboard sync (e.g. uploadedProjects)
      ...existingFbWs,
      // Overwrite only the four fields the dashboard owns
...
app.post('/api/upload-project', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // ── Sync to Firebase BEFORE responding so the client knows it persisted ────
    ...
    try {
      ...
      if (firebaseSynced) {
        ...
        console.warn('[upload-project] writeWorkspaceToFirebase returned false for:', req.user.email);
      }
    } catch(fbErr) {
      console.warn('[upload-project] Firebase sync failed:', fbErr.message);
    }
