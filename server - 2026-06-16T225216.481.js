...
// If step 1 is momentarily slow (Firebase cold start, cache refresh timing out),
// Cloudflare sees a timeout and shows a 502 — even though the server is healthy
// and the in-process watchdog's heartbeat is passing fine.
...
async function executeAgentTool(toolName, toolArgs, sandboxDir, readFilesTracker, taskState) {
  ...
  try {
    ...
    if (toolName==='execute_command') {
      ...
      const BLOCKED=['npm start','node server','nodemon','python -m http','flask run','rails s','rm -rf /','forever ','pm2 start'];
      if(BLOCKED.some(b=>cmd.toLowerCase().includes(b.toLowerCase())))
        return{success:false,error:`Command blocked: "${cmd.slice(0,80)}". Use file tools to make changes instead.`};
...
// ── Main agent runner ─────────────────────────────────────────────
async function runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider }) {
  ...
  try {
    ...
    if (hasChanges) {
      ...
      await execAsync(
        `cd "${tmpDir}" && git add -A && git commit -m "fix: JoyTree AI (${getAgentProviderLabel(provider)}) [${deploymentId ? deploymentId.slice(-8) : 'manual'}]\n\n${finalSummary.slice(0, 400)}" 2>&1`,
        { timeout: 20000 }
      );
    ...
    if (tmpDir) execAsync(`rm -rf "${tmpDir}"`).catch(() => {});
  }
}
...
app.post('/api/ai/agent/start', requireAuth, async (req, res) => {
  const { repoSlug, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider } = req.body || {};
  ...
  runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider }).catch(e => {
