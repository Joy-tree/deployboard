...
// GET /api/v1/ssh-keys — list all SSH keys for the authenticated user
...
// POST /api/v1/ssh-keys — generate a new SSH key pair
...
// DELETE /api/v1/ssh-keys/:keyId — revoke and remove an SSH key
        ...
        if (rec?.publicKey) {
          ...
          try {
            ...
            if (keyBody) {
              ...
              fs.writeFileSync(authKeysPath, filtered, 'utf8');
            }
          } catch (_) {}
        }
      }
...
// GET /api/v1/github/repos — list linked GitHub repositories
v1.get('/github/repos', async (req, res) => {
  try {
    ...
    for (let page = 1; page <= 3; page++) {
      ...
      const data = await r.json();
      if (!r.ok) return res.status(r.status).json({ ok: false, error: data?.message || 'GitHub fetch failed' });
      if (!Array.isArray(data) || !data.length) break;
      all = all.concat(data);
      if (data.length < 100) break;
    }
...
// GET /api/v1/github/branches — list branches for a repo URL
...
// GET /api/v1/domains — list user's custom domains
...
// POST /api/v1/domains/attach — attach a custom domain to a project
v1.post('/domains/attach', async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      ws.customDomains.push({ domain, projectId, verified: false, createdAt: new Date().toISOString() });
    }
    await writeWorkspaceToFirebase(req.user, ws);
...
// POST /api/v1/domains/:domain/verify — trigger DNS verification
v1.post('/domains/:domain/verify', async (req, res) => {
  try {
    ...
    // Simple DNS check — resolve CNAME and see if it points to our base domain
    const dns = require('dns').promises;
    let verified = false;
    try {
      const records = await dns.resolveCname(domain);
      verified = records.some(r => r.includes(BASE_DOMAIN));
    } catch (_) {}
...
// DELETE /api/v1/domains/:domain — remove a custom domain
...
// GET /api/v1/domains/check?domain=example.com — check domain availability
v1.get('/domains/check', async (req, res) => {
  try {
    ...
    try {
      ...
      available = false; // domain resolves = taken
    } catch (e) {
      available = e.code === 'ENOTFOUND'; // not found = available
    }
...
// GET /api/v1/webhook/secret — show the global webhook secret
v1.get('/webhook/secret', async (req, res) => {
  res.json({
    ok:           true,
    secret:       GLOBAL_WEBHOOK_SECRET || '',
    webhookUrl:   `https://${BASE_DOMAIN}/api/github/webhook`,
    note:         'Use this secret for all GitHub repository webhooks.'
  });
});

// POST /api/v1/webhook/rotate — regenerate the global webhook secret
v1.post('/webhook/rotate', async (req, res) => {
  ...
  res.json({
    ...
    secret:     GLOBAL_WEBHOOK_SECRET,
    webhookUrl: `https://${BASE_DOMAIN}/api/github/webhook`,
    note:       'Secret rotated. Update your GitHub webhook settings and add GLOBAL_WEBHOOK_SECRET to your .env.'
  });
});

// ── Autodeploy ────────────────────────────────────────────────────────────────

// PATCH /api/v1/projects/:id/autodeploy — enable or disable auto-deploy
...
// ── Databases — extra v1 wrappers ─────────────────────────────────────────────

// POST /api/v1/databases — create a new database
v1.post('/databases', async (req, res) => {
  try {
    // Proxy to the internal session route using the internal key
...
// POST /api/v1/databases/:id/delete
...
// POST /api/v1/apikey/rotate — rotate the user's API key
