...
async function buildFlowWithGroq({ prompt = '', sourceText = '', fileName = '' } = {}) {
  ...
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), Number(process.env.GROQ_FLOW_TIMEOUT_MS || 45000));
    let r;
    try {
      r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.3,
          max_completion_tokens: Number(process.env.GROQ_FLOW_MAX_TOKENS || 2600),
          top_p: 1,
          stream: false
        })
      });
...
async function buildFlowWithAI({ prompt = '', sourceText = '', fileBase64 = '', fileMime = '', fileName = '' } = {}) {
  // Joytree AI — powered by server-side AI exclusively
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  const providerRequested = 'groq'; // always Groq
  ...
  if (!aiSpec) return res.status(502).json({ error: aiError || 'Groq did not return a valid response. Try rephrasing your prompt.' });
  if (!aiSpec.responseTemplate || typeof aiSpec.responseTemplate !== 'object') return res.status(502).json({ error: 'Groq returned an invalid output structure. Try rephrasing your prompt.' });
  const usedProvider = 'joytree';
  const usedModel = 'Joytree AI';
...
app.post('/api/databases/:id/query', requireAuth, async (req, res) => {
  try {
    ...
    if (!result.ok) {
      const errMsg = (result.stderr || result.stdout || 'Query execution failed').trim();
