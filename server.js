...
const GLOBAL_WEBHOOK_SECRET = process.env.GLOBAL_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET || '';
...
app.post('/api/projects/:id/autodeploy', requireAuth, async (req, res) => {
  try {
    ...
    await project.save();
    res.json({ ok:true, enabled: project.autoDeployEnabled, webhookUrl: `${req.protocol}://${req.get('host')}/api/github/webhook`, secret: project.autoDeploySecret });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
