...
function ideTermSpawn(sessionId, socket, userId, projectId) {
  ...
  try {
    ...
    proc = cp.spawn(shell, shell.includes('bash') ? ['--norc', '--noprofile'] : [], {
      cwd,
      env,
      stdio: ['pipe', 'pipe', 'pipe'],
      detached: false,
  ...
  ideTermSessions.set(sessionId, { proc, socket, userId, bytesThisSec, rateTimer, memTimer, idleTimer, get lastActivity() { return lastActivity; }, set lastActivity(v) { lastActivity = v; } });
  // Set prompt and trigger first prompt display.
  // Do NOT run 'clear' — it emits garbage escape codes on a non-TTY pipe.
  proc.write('export PS1="joytree:\\w# "; echo\n');
