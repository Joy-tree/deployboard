...
app.post('/api/upload-project', requireAuth, async (req, res) => {
  ...
  busboy.on('finish', async () => {
    ...
    try {
      ...
      // Persist upload metadata to Firebase workspace so it survives server restarts
      try {
      ...
      } catch(fbErr) { console.warn('[upload-project] Firebase metadata save failed:', fbErr.message); }
