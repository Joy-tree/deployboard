...
// ===== CRASH GUARD — keeps the process alive on any unhandled error =====
process.on('uncaughtException', (err) => {
  console.error('[CRASH GUARD] uncaughtException — server kept alive:', err && err.stack || err);
});
process.on('unhandledRejection', (reason) => {
  console.error('[CRASH GUARD] unhandledRejection — server kept alive:', reason && reason.stack || reason);
});
// ========================================================================
...
function proxyResponseOrJsonMismatch(req, res, proxyRes, subdomain) {
  // Pass ALL responses from server apps straight through to the client.
  // Do NOT intercept HTML responses — server apps like Next.js legitimately
  // return HTML for pages, error boundaries, and SSR routes. Intercepting
  // HTML for "programmatic" requests was causing the DOCTYPE JSON parse error
  // in the browser when an app's API route threw an error (Next.js renders
  // the error as an HTML page which we were replacing with a fake JSON error).
...
const CD_CACHE_TTL_MS = 30 * 1000; // refresh every 30 seconds
...
app.use(async (req, res, next) => {
  ...
  try {
    ...
    if (Date.now() - _cdCacheLastRefresh > CD_CACHE_TTL_MS) {
      refreshCustomDomainCache().catch(() => {}); // non-blocking
      _cdCacheLastRefresh = Date.now(); // optimistic update to avoid stampede
    }
...
// ── GET /api/projects/:id/runtime-logs  (SSE — live container logs) ───────────
// Streams `docker logs --follow` output for a deployed server app to the
// browser as Server-Sent Events. Works exactly like Render's "Logs" tab.
// Query params:
//   ?lines=200   — how many historical lines to include before tailing (default 200)
app.get('/api/projects/:id/runtime-logs', requireAuth, async (req, res) => {
  try {
    const userId = String(req.user._id || req.user.id);
    let project = null;
    if (isDbReady()) project = await findProjectByAnyId(req.params.id);
    if (!project) {
      const ws = (await readWorkspaceFromFirebase(req.user)) || {};
      const wsProjects = Array.isArray(ws.projects) ? ws.projects : [];
      project = wsProjects.find(p =>
        String(p.id || p._id || '') === String(req.params.id) ||
        String(p.subdomain || '') === String(req.params.id)
      ) || null;
    }
    if (!project) return res.status(404).json({ error: 'Project not found' });

    const projectOwner = String(project.ownerUserId || project.userId || '');
    if (projectOwner && projectOwner !== userId) return res.status(403).json({ error: 'Forbidden' });

    const subdomain = String(project.subdomain || '');
    reloadPortRegistryFromDisk();
    const entry = portRegistry[subdomain];
    if (!entry || !String(entry).includes(':')) {
      // Not a server app or not yet deployed
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      res.flushHeaders();
      res.write('data: {"line":"[runtime] No running container found — deploy a server app first."}\n\n');
      res.end();
      return;
    }

    // entry is "db-nameee:3002" — extract the container name
    const containerName = String(entry).split(':')[0];

    const lines = Math.min(500, Math.max(1, Number(req.query.lines || 200)));
    const { spawn } = require('child_process');
    const child = spawn('docker', ['logs', '--follow', `--tail=${lines}`, containerName], {
    ...
    // Send a keepalive comment every 15s so the browser doesn't disconnect
    const keepalive = setInterval(() => { try { res.write(': keepalive\n\n'); } catch(_){} }, 15000);

    const sendLine = (line) => {
      try {
        const trimmed = String(line).replace(/\r/g, '');
        if (!trimmed) return;
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        res.write('data: ' + JSON.stringify({ line: `[runtime] Container stream ended (exit ${code ?? '?'}).` }) + '\n\n');
        res.end();
      } catch(_) {}
    });
    ...
    child.on('error', (err) => {
      clearInterval(keepalive);
      try {
        res.write('data: ' + JSON.stringify({ line: `[runtime] Error: ${err.message}` }) + '\n\n');
        res.end();
      } catch(_) {}
    });
...
app.post('/api/deploy', requireAuth, async (req, res) => {
  ...
  // Use the AUTHENTICATED user's id as the primary room target.
  // project.ownerUserId can be a stale temp 'u_<timestamp>' from Firebase-only
  // mode that won't match the MongoDB _id the client sends in auth:join.
  // Falling back to req.user._id ensures the room always matches the socket.
  const _authUserId   = String(req.user?._id || req.user?.id || '');
  const _ownerRoomId  = _authUserId || String(project?.ownerUserId || '');
  // Emit to BOTH the deploy-specific room AND the user's private room.
  // The deploy room (subscribe:deploy) guarantees delivery even when auth:join
  // fires before currentUser is set on the client (window.onload race).
  const emit = (event, data) => {
    const payload = { deployId, projectId: String(project._id), source: deploySource, triggerSha, ...data };
    io.to('deploy:' + deployId).emit(event, payload);
    if (_ownerRoomId) io.to('user:' + _ownerRoomId).emit(event, payload);
...
io.on('connection', socket => {
  ...
  // Client sends its userId so we can route build events to the right user only
  socket.on('auth:join', ({ userId } = {}) => {
    if (userId) {
      socket.join('user:' + String(userId));
      socket._joytreeUserId = String(userId);
      console.log('[Socket.io] auth:join: socket', socket.id, '→ room user:' + userId);
    }
  });
  // Client subscribes to a SPECIFIC deployment's log stream by deployId.
  // This is the guaranteed fallback: even if auth:join fires before currentUser
  // is set (window.onload race), the client subscribes right after POST /deploy
  // returns the deployId, so no log lines are ever missed.
  socket.on('subscribe:deploy', ({ deployId } = {}) => {
    if (deployId) {
      socket.join('deploy:' + String(deployId));
      socket._joytreeDeployId = String(deployId);
      console.log('[Socket.io] subscribe:deploy: socket', socket.id, '→ room deploy:' + deployId);
    }
  });
  socket.on('unsubscribe:deploy', ({ deployId } = {}) => {
    if (deployId) socket.leave('deploy:' + String(deployId));
  });
...
app.post('/api/upload-deploy', requireAuth, async (req, res) => {
  ...
  const emit = (event, data) => {
    const payload = { deployId: finalDeployId, projectId: cleanSub, source: 'upload', ...data };
    io.to('deploy:' + finalDeployId).emit(event, payload);
    if (_uploadOwnerRoomId) io.to('user:' + _uploadOwnerRoomId).emit(event, payload);
    else io.emit(event, payload);
  };
...
app.post('/api/projects/:id/redeploy-upload', requireAuth, async (req, res) => {
  ...
  const emit = (event, data) => {
    const payload = { deployId: finalDeployId, projectId: cleanSub, source: 'upload', ...data };
    io.to('deploy:' + finalDeployId).emit(event, payload);
    if (_redeployOwnerRoomId) io.to('user:' + _redeployOwnerRoomId).emit(event, payload);
    else io.emit(event, payload);
  };
...
app.get('/api/account/api-key', requireAuth, async (req, res) => {
  try {
    ...
    // If no snapshot exists yet, seed it now
...
v1.post('/projects/:id/restart', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({ ok: false, error: `Docker restart failed: ${dockerErr.message}` });
    }
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
v1.get('/projects/:id/status', async (req, res) => {
  try {
    ...
    let stats = {};
    try {
      const out = execSync(`docker inspect --format '{{.State.Status}}' ${containerName} 2>/dev/null`, { encoding: 'utf8', timeout: 5000 }).trim();
      containerStatus = out || 'unknown';
      if (containerStatus === 'running') {
        const s = execSync(`docker stats --no-stream --format '{"cpu":"{{.CPUPerc}}","mem":"{{.MemUsage}}","netIn":"{{.NetIO}}","block":"{{.BlockIO}}"}' ${containerName} 2>/dev/null`, { encoding: 'utf8', timeout: 8000 }).trim();
        try { stats = JSON.parse(s); } catch (_) {}
      }
    } catch (_) {}
    res.json({ ok: true, project: project.subdomain, projectStatus: project.status || 'unknown', container: containerStatus, stats, liveUrl: project.liveUrl || `https://${project.subdomain}.${BASE_DOMAIN}` });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
...
server.listen(PORT, () => {
  ...
  // Clean up any leftover candidate containers from crashed/interrupted builds.
  // These accumulate as zombies and waste memory. Run once on startup then every hour.
  const cleanupCandidates = () => {
    try {
      const { execSync } = require('child_process');
      // List all stopped/restarting containers with "cand-local" in the name
      const out = execSync(
        'docker ps -a --filter "name=cand-local" --filter "status=exited" --filter "status=dead" --format "{{.Names}}" 2>/dev/null || true',
        { encoding: 'utf8', timeout: 15000 }
      ).trim();
      if (out) {
        const names = out.split('\n').filter(Boolean);
        if (names.length) {
          execSync(`docker rm -f ${names.join(' ')} 2>/dev/null || true`, { timeout: 30000 });
          console.log(`[Cleanup] Removed ${names.length} stale candidate containers`);
        }
      }
    } catch(_) {}
  };
  setTimeout(cleanupCandidates, 5000);
  setInterval(cleanupCandidates, 60 * 60 * 1000); // every hour
});

// ── WebSocket upgrade proxy ──────────────────────────────────────────────────
// Apps like Uptime Kuma, Socket.io apps, and Next.js HMR use WebSocket.
// Without this handler the upgrade request is dropped and the app shows a
// blank page or fails to connect to its own backend.
server.on('upgrade', (req, socket, head) => {
  try {
    ...
    const target = parseProxyTarget(entry);
    if (!target) return socket.destroy();

    const netMod = require('net');
    const upstream = netMod.connect(target.port, target.host, () => {
      // Replay the upgrade request to the upstream container
...
// Monitor total Node.js process RSS — if it climbs too high, kill oldest sessions
...
setInterval(() => {
  try {
    ...
    if (rssMB > IDE_HOST_MEM_WARN_MB && ideTermSessions.size > 0) {
      // Kill the oldest session to free memory
...
function ideTermSpawn(sessionId, socket, userId, projectId) {
  ...
  // Priority: 1) uploads dir (IDE editor files), 2) deployed site dir, 3) home
...
io.on('connection', (socket) => {
  socket.on('ide:term:start', (data) => {
    try {
      const sessionId = ideTermGenId();
      const userId    = String(data?.userId || '');
      ideTermSpawn(sessionId, socket, userId, String(data?.projectId || ''));
    } catch (err) {
