...
async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  ...
  for (let chunk = 0; chunk < totalChunks; chunk++) {
    ...
    let providers;
    if (aiVersion === 'v4') {
      const v4Providers = getV4ProviderOrder(chunk);
      providers = v4Providers.length ? v4Providers : ['groq'];
    } else if (aiVersion === 'v3') {
      providers = ['cerebras'];
    } else {
      providers = aiVersion === 'v2' && XAI_API_KEY ? ['groq', 'xai'] : ['groq'];
    }
