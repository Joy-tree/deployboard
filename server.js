...
async function provisionDbContainer(db) {
  ...
  if (db.engine === 'mysql' || db.engine === 'mariadb') {
    ...
    while (Date.now() - started < maxWaitMs) {
      ...
      const pingCmd = 'docker exec ' + containerName + ' ' + mysqlBin2 +
        ' -h 127.0.0.1 -u"' + (db.user || 'root') + '" -p"' + (db.pass || '') +
        '" --batch --silent -e "SELECT 1" 2>&1';
      const ping = await runDocker(pingCmd, 5000);
