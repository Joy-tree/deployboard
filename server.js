...
// Joytree API v4 — Multi-AI cascade (Groq + xAI) with silent failover and web search
// Groq and xAI keys are already declared above (GROQ_API_KEY / XAI_API_KEY).
// These aliases are kept so the v4 helpers remain self-documenting.
const V4_GROQ_API_KEY  = GROQ_API_KEY;
const V4_GROQ_MODEL    = GROQ_MODEL;
const V4_XAI_API_KEY   = XAI_API_KEY;
const V4_XAI_MODEL     = XAI_MODEL;
// V4 web-search timeout (ms) — reuses JOYTREE_WEB_SEARCH_URL env var
const V4_WEB_SEARCH_TIMEOUT_MS = 8000;
...
async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  ...
  for (let chunk = 0; chunk < totalChunks; chunk++) {
    ...
    // v4 = Groq+xAI cascade; v3 = Cerebras; v2 = Groq+xAI; v1 = Groq only
...
// v4 web-search helper — uses JOYTREE_WEB_SEARCH_URL (same as v3 web search)
// Injects live search snippets into the prompt so both Groq and xAI get web context.
async function fetchV4WebSearchContext(query = '') {
  if (!JOYTREE_WEB_SEARCH_URL || !query) return '';
  try {
    const controller = new AbortController();
    const tid = setTimeout(function(){ controller.abort(); }, V4_WEB_SEARCH_TIMEOUT_MS);
    try {
      const r = await fetch(JOYTREE_WEB_SEARCH_URL + '?q=' + encodeURIComponent(String(query).slice(0, 300)), {
        signal: controller.signal,
        headers: { 'Content-Type': 'application/json' }
      });
      if (!r.ok) return '';
      const data = await r.json().catch(function(){ return {}; });
      // Accept array of {title,snippet,url} or {results:[...]} or plain string
      const items = Array.isArray(data) ? data : (Array.isArray(data && data.results) ? data.results : []);
      if (!items.length) return '';
      return '\n\nWEB_SEARCH_RESULTS:\n' + items.slice(0, 5).map(function(i, n){
        return (n + 1) + '. ' + String(i.title || '') + '\n' + String(i.snippet || i.description || '') + (i.url ? '\nSource: ' + i.url : '');
      }).join('\n\n');
    } finally {
      clearTimeout(tid);
    }
  } catch(e) {
    console.warn('[v4 web-search] fetch failed:', e.message);
    return '';
  }
}

// v4 Groq caller (OpenAI-compatible)
async function callV4GroqChat({ messages, maxTokens = 4096, timeoutMs = 90000, temperature = 0.3 } = {}) {
  if (!V4_GROQ_API_KEY) throw new Error('GROQ_API_KEY is not configured for v4.');
  const controller = new AbortController();
  const tid = setTimeout(function(){ controller.abort(); }, timeoutMs);
  try {
    const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      signal: controller.signal,
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + V4_GROQ_API_KEY },
      body: JSON.stringify({
        model: V4_GROQ_MODEL,
        messages: messages,
        max_tokens: maxTokens,
    ...
    if (!r.ok) {
      ...
      console.error('[v4 Groq] API error', r.status, ':', msg);
      throw new Error('v4 Groq HTTP ' + r.status + ': ' + msg);
    }
    const data = await r.json().catch(function(){ return {}; });
    const text = String((data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) || '');
    if (!text) throw new Error('v4 Groq returned empty content.');
    return text;
  } catch(e) {
    if (e.name === 'AbortError') throw new Error('v4 Groq timed out after ' + Math.round(timeoutMs / 1000) + 's.');
    throw e;
  } finally {
    clearTimeout(tid);
  }
...
// v4 xAI caller (OpenAI-compatible)
async function callV4XAIChat({ messages, maxTokens = 4096, timeoutMs = 120000, temperature = 0.3 } = {}) {
  if (!V4_XAI_API_KEY) throw new Error('XAI_API_KEY is not configured for v4.');
  const controller = new AbortController();
  const tid = setTimeout(function(){ controller.abort(); }, timeoutMs);
  try {
    const r = await fetch('https://api.x.ai/v1/chat/completions', {
      method: 'POST',
      signal: controller.signal,
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + V4_XAI_API_KEY },
      body: JSON.stringify({
        model: V4_XAI_MODEL,
        messages: messages,
        max_tokens: maxTokens,
    ...
    if (!r.ok) {
      ...
      const j = (function(){ try { return JSON.parse(t); } catch { return null; } })();
      const msg = (j && j.error && j.error.message) || t.slice(0, 300) || ('HTTP ' + r.status);
      console.error('[v4 xAI] API error', r.status, ':', msg);
      throw new Error('v4 xAI HTTP ' + r.status + ': ' + msg);
    }
    const data = await r.json().catch(function(){ return {}; });
    const text = String((data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) || '');
    if (!text) throw new Error('v4 xAI returned empty content.');
    return text;
  } catch(e) {
    if (e.name === 'AbortError') throw new Error('v4 xAI timed out after ' + Math.round(timeoutMs / 1000) + 's.');
    throw e;
  } finally {
    clearTimeout(tid);
  }
...
// v4 single-shot flow builder — Groq first, silent failover to xAI, web search injected
async function buildFlowWithV4({ prompt = '', sourceText = '', fileName = '' } = {}) {
  // Inject live web search context into the source text if search is enabled
  const searchSnippets = await fetchV4WebSearchContext(String(prompt || '').slice(0, 300));
  const augmentedSource = String(sourceText || '') + searchSnippets;

  const userInput = 'PROMPT:\n' + String(prompt || '').slice(0, 20000) + '\n\nSOURCE_TEXT:\n' + augmentedSource.slice(0, 20000) + '\n\nFILE_NAME:\n' + String(fileName || '').slice(0, 200);
  const systemMsg = AI_FLOW_INSTRUCTION + '\nJoytree API v4 uses multi-AI cascade (Groq + xAI). If WEB_SEARCH_RESULTS appear in SOURCE_TEXT, use them as current context.';
  const messages = [{ role: 'system', content: systemMsg }, { role: 'user', content: userInput }];

  // Try Groq first
  try {
    const text = await callV4GroqChat({ messages: messages, temperature: 0.25, maxTokens: 8192 });
    const parsed = extractJsonFromText(text);
    if (parsed) return Object.assign({}, parsed, { _aiProvider: 'groq', _aiModel: 'Joytree API v4' });
    throw new Error('Groq returned non-JSON output.');
  } catch(e) {
    console.warn('[v4] Groq flow build failed:', e.message, '— falling back to xAI');
  }
  // Fallback to xAI
  const text2 = await callV4XAIChat({ messages: messages, temperature: 0.25, maxTokens: 8192 });
  const parsed2 = extractJsonFromText(text2);
  if (!parsed2) throw new Error('xAI also returned non-JSON output. Try rephrasing your prompt.');
  return Object.assign({}, parsed2, { _aiProvider: 'xai', _aiModel: 'Joytree API v4' });
}
...
async function callV4Chunk(messages) {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    try {
      let text;
      if (provider === 'groq') {
        text = await callV4GroqChat({ messages: messages, temperature: 0.7, maxTokens: 4096 });
      } else {
        text = await callV4XAIChat({ messages: messages, temperature: 0.7, maxTokens: 4096 });
      }
  ...
  throw new Error('v4 chunk ' + chunkIndex + ': both Groq and xAI failed. Check API keys and quotas.');
}
...
async function buildFlowWithAI({ prompt = '', sourceText = '', fileBase64 = '', fileMime = '', fileName = '', aiProvider = 'auto', aiVersion = 'v1' } = {}) {
  // Joytree API v4 — Groq + xAI cascade with web search
  if (aiVersion === 'v4') {
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  // v4 always gets web search context (DuckDuckGo for live web info)
