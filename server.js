...
app.post('/api/databases/:id/query', requireAuth, async (req, res) => {
  try {
    ...
    // [FIX] MongoDB: check for empty-DB success BEFORE the generic isErrorOutput
    // test. listCollections on an empty database returns:
    //   { cursor: { firstBatch: [], ...}, ok: 1 }
    // ok:1 is a SUCCESS — the DB is just empty. We must return this as a
    // success with emptyDb:true so the frontend shows "no collections yet"
    // instead of a confusing empty or false-error state.
    if (db.engine === 'mongodb' && rawOut) {
      try {
        const _mParsed = JSON.parse(rawOut);
        if (_mParsed?.ok === 1 && Array.isArray(_mParsed?.cursor?.firstBatch) && _mParsed.cursor.firstBatch.length === 0) {
          addActivity('database', `🔍 Query on "${db.name}" (mongodb) — empty database`);
          return res.json({ ok: true, rows: [], raw: rawOut, engine: 'mongodb', emptyDb: true, message: 'No collections found — database is empty. Insert a document or use db.createCollection() to get started.' });
        }
      } catch (_) {}
    }
