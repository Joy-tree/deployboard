...
app.use((req, res, next) => {
  // Skip express.json for multipart upload routes — busboy needs the raw stream
...
// ── Multipart upload ─────────────────────────────────────────────────────────
app.post('/api/upload-project', requireAuth, (req, res) => {
  ...
  // Resolve busboy regardless of whether it exports a class or a factory function
  let BusboyMod;
  try { BusboyMod = require('busboy'); } catch(e) {
    return res.status(500).json({ error: 'busboy module not installed on server. Run: npm install busboy' });
  }
  const busboyFactory = (typeof BusboyMod === 'function') ? BusboyMod : BusboyMod.default || BusboyMod;
  let busboy;
  try { busboy = busboyFactory({ headers: req.headers, limits: { fileSize: 260 * 1024 * 1024 } }); }
  catch(e) { return res.status(500).json({ error: 'Failed to init busboy: ' + e.message }); }
  ...
  busboy.on('file', (fieldname, fileStream, info) => {
    ...
    try { fs.mkdirSync(userUploadDir, { recursive: true }); } catch(e) {
      ...
      return sendErr(500, 'Could not create upload directory: ' + e.message);
    }
    savedArchivePath = path.join(userUploadDir, origFilename);
    const ws = fs.createWriteStream(savedArchivePath);
    fileStream.pipe(ws);
    writeFinishPromise = new Promise((resolve, reject) => {
      ws.on('finish', resolve);
      ws.on('error', reject);
      fileStream.on('error', reject);
      // Handle busboy fileSize limit truncation
      fileStream.on('limit', () => reject(new Error('File exceeds 250 MB limit')));
    });
  });
  ...
  busboy.on('finish', async () => {
    if (responseSent) return;
    try {
      if (writeFinishPromise) await writeFinishPromise;
    } catch(e) {
      return sendErr(500, 'File write failed: ' + e.message);
    }
    if (!savedArchivePath || !fs.existsSync(savedArchivePath)) {
      return sendErr(400, 'No file received — make sure you selected a .zip, .tar.gz, or .tgz file');
    }
    const fileSize = fs.statSync(savedArchivePath).size;
    if (fileSize === 0) {
      fs.unlinkSync(savedArchivePath);
      return sendErr(400, 'Uploaded file is empty');
    }
    try {
      const result = await extractUploadedArchive(savedArchivePath, path.join(UPLOADS_DIR, userId, projectId, 'files'));

      // ── Respond to the client immediately — never block on Firebase ──────────
      if (!responseSent) {
        responseSent = true;
        res.json({
          ok: true, projectId,
      ...
      // ── Sync metadata to Firebase in the background ───────────────────────────
      setImmediate(async () => {
        try {
          const userForFirebase = await enrichAuthUser(req.user);
          const meta = {
            id: projectId, name: projectName,
            fileName: path.basename(savedArchivePath),
            lang: result.lang, fileCount: result.fileCount,
            size: result.unzippedSize,
            uploadedAt: new Date().toISOString(),
            extracted: true,
            serverPath: path.join(UPLOADS_DIR, userId, projectId, 'files')
          };
          let ws = null;
          try { ws = await readWorkspaceFromFirebase(userForFirebase); } catch { ws = null; }
          ...
          console.warn('[upload-project] Firebase background sync failed:', fbErr.message);
        }
      });
    } catch(e) {
      return sendErr(500, 'Extraction failed: ' + e.message);
    }
  });

  busboy.on('error', e => sendErr(500, 'Upload error: ' + e.message));
  req.on('error', e => sendErr(500, 'Request stream error: ' + e.message));
  req.pipe(busboy);
});
...
app.post('/api/upload-deploy', requireAuth, async (req, res) => {
  ...
  (async () => {
    try {
      ...
      emit('build:log', { line: `\n\x1b[32m✓ Deployed in ${duration}s\x1b[0m` });
      await saveDeployStatus('success', { duration, endedAt: new Date().toISOString(), liveUrl: cf?.url || `https://${cleanSub}.${BASE_DOMAIN}` });
      ...
      const duration = Math.round((Date.now() - buildStart) / 1000);
      emit('build:log', { line: `\x1b[31m[Joytree]\x1b[0m Build failed: ${safeErr}` });
      await saveDeployStatus('failed', { duration, endedAt: new Date().toISOString(), error: safeErr });
