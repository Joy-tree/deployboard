...
app.post('/api/databases/:id/query', requireAuth, async (req, res) => {
  try {
    ...
    switch (db.engine) {
      ...
      case 'mysql':
      case 'mariadb': {
        // [FIX] Use stdin piping instead of -e '...' so multi-statement scripts work.
        // -e '...' on MariaDB CLI does NOT support multiple semicolon-separated statements
        // reliably (it silently stops after the first one). Piping via echo "..." | docker exec -i
        // uses the client's --multi-statements mode automatically when reading from stdin.
        // We shell-escape the query for use inside $'...' (ANSI-C quoting), which safely
        // handles single quotes, newlines, and special chars without the escaping mess.
        const mysqlBin = db.engine === 'mariadb' ? 'mariadb' : 'mysql';
        // Escape backslashes and single quotes for safe embedding in shell $'...' string
        const safeQ = query.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        cmd = `docker exec -i ${db.containerName} ${mysqlBin} -u"${db.user||'root'}" -p"${db.pass}" "${db.dbName||'mydb'}" --batch --silent 2>&1 <<< $'${safeQ}'`;
        break;
      }
      case 'redis': {
