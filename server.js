...
function externalDbConnStr(db){
  ...
  // Use the real host port (what's actually bound on the VPS) — containers are on random
  // ports in the 14000-15000 range so users must connect on that port, not the standard one.
  ...
  switch (db.engine){
    ...
    case 'mongodb':  return `mongodb://${user}:${pass}@${host}:${port || 27017}/${name}`;
...
app.post('/api/databases/:id/query', requireAuth, async (req, res) => {
  try {
    ...
    switch (db.engine) {
      ...
      case 'mariadb': {
        const safeQ = query.replace(/'/g, "'\\''").replace(/\n/g, ' ');
        // mariadb:11 dropped the 'mysql' alias — use 'mariadb' CLI for mariadb engine,
        // keep 'mysql' for mysql:8 which still ships both binaries.
        const mysqlBin = db.engine === 'mariadb' ? 'mariadb' : 'mysql';
        cmd = `docker exec ${db.containerName} ${mysqlBin} -u"${db.user||'root'}" -p"${db.pass}" "${db.dbName||'mydb'}" --batch --silent -e '${safeQ}' 2>&1`;
        break;
      }
      case 'redis': {
...
app.post('/api/ssh-keys/generate', requireAuth, express.json(), async (req, res) => {
  try {
    ...
    const tmpPriv = path.join(os.tmpdir(), `jtkey_${Date.now()}_priv`);
    const tmpPub  = path.join(os.tmpdir(), `jtkey_${Date.now()}_pub`);

    // Generate ed25519 keypair using ssh-keygen (available on all Linux VPS)
    const { execAsync: _ea } = (() => {
      const { promisify } = require('util');
      return { execAsync: promisify(require('child_process').exec) };
    })();

    await _ea(`ssh-keygen -t ed25519 -C "${safeLabel}" -f "${tmpPriv}" -N "" -q 2>&1`);

    const privateKey = fs.readFileSync(tmpPriv, 'utf8');
    const publicKey  = fs.readFileSync(tmpPub + '.pub', 'utf8').trim();  // ssh-keygen appends .pub

    // Install public key into ~/.ssh/authorized_keys
    ...
    if (!existing.includes(publicKey.split(' ')[1])) {  // compare key body, not comment
      fs.appendFileSync(authKeys, '\n' + publicKey + '\n', 'utf8');
      fs.chmodSync(authKeys, 0o600);
    }

    // Clean up temp files
    try { fs.unlinkSync(tmpPriv); fs.unlinkSync(tmpPriv + '.pub'); } catch (_) {}

    res.json({
      ok: true,
      label: safeLabel,
      publicKey,
      privateKey,   // returned once — client must save it
      fingerprint: await _ea(`ssh-keygen -lf /dev/stdin <<< "${publicKey}" 2>/dev/null || echo "ed25519"`).then(r => r.stdout.trim()).catch(() => 'ed25519'),
...
// GET /api/ssh-keys/list — list public keys from authorized_keys
app.get('/api/ssh-keys/list', requireAuth, async (req, res) => {
  try {
    const authKeys = path.join(process.env.HOME || '/root', '.ssh', 'authorized_keys');
    let lines = [];
    try { lines = fs.readFileSync(authKeys, 'utf8').split('\n').filter(l => l.trim() && !l.startsWith('#')); } catch (_) {}
    const keys = lines.map((line, i) => {
      const parts = line.trim().split(' ');
      return {
        index: i,
        type:    parts[0] || '',
        keyBody: parts[1] ? parts[1].slice(0, 24) + '…' : '',
        label:   parts[2] || '',
        line:    line.trim(),
      };
    });
...
// DELETE /api/ssh-keys/:index — remove a key by its line index
app.delete('/api/ssh-keys/:index', requireAuth, async (req, res) => {
  try {
    if (!isRootEmailAdmin(req.user)) return res.status(403).json({ error: 'Admin only' });
    const authKeys = path.join(process.env.HOME || '/root', '.ssh', 'authorized_keys');
    let lines = [];
    try { lines = fs.readFileSync(authKeys, 'utf8').split('\n'); } catch (_) {}
    const idx = parseInt(req.params.index, 10);
    const filtered = lines.filter((l, i) => {
      if (!l.trim() || l.startsWith('#')) return true;  // keep blanks/comments
      let keyIdx = -1;
      let count  = 0;
      for (let j = 0; j < lines.length; j++) {
        if (lines[j].trim() && !lines[j].startsWith('#')) { if (count === idx) { keyIdx = j; break; } count++; }
      }
      return i !== keyIdx;
    });
    // Simpler: filter by index in the non-comment lines
    const nonEmpty = lines.filter(l => l.trim() && !l.startsWith('#'));
    if (idx < 0 || idx >= nonEmpty.length) return res.status(404).json({ error: 'Key not found' });
    const toRemove = nonEmpty[idx];
    const newContent = lines.filter(l => l !== toRemove).join('\n');
    fs.writeFileSync(authKeys, newContent, 'utf8');
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
