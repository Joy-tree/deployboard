...
async function runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext, sourceType, uploadProjectId, userId }) {
  ...
  try {
    ...
    session.runCtx = { provider, repoSlug, projectName, deploymentId, githubToken, errorText, sourceType, uploadProjectId, userId, branchName: session.branchName };
    session.iteration = 0;
    ...
    if (!session.paused && tmpDir && tmpDir.startsWith(path.join(os.tmpdir(), 'joytree-agent-'))) {
...
async function resumeAgentSession(sessionId) {
  ...
  if (!session.paused || !session.messages || !session.runCtx) throw new Error('This session cannot be resumed.');
  ...
  try {
    ...
    while (iteration < MAX_ITERATIONS) {
      ...
      let groqData;
      try {
        groqData = await callAgentLLM({ provider, messages, tools: GROQ_AGENT_TOOL_SCHEMAS, temperature: 0.2, maxTokens: 8000 });
      } catch (llmErr) {
        const m = llmErr.message || String(llmErr);
        if (m.startsWith('__JAI_RATE_LIMIT__:')) {
          const parts = m.split(':');
