...
const CHUNK_SIZE = 25; // small chunks so each call stays within Groq rate limits (~800 tokens per chunk)
...
async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  ...
  for (let chunk = 0; chunk < totalChunks; chunk++) {
    ...
    // Gap between chunks to avoid rate-limit hammering — Groq free tier is ~6k tokens/min
    if (chunk < totalChunks - 1) await new Promise(r => setTimeout(r, 1500));
  }
