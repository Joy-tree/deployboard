...
app.post('/api/upload-project', requireAuth, async (req, res) => {
  ...
  busboy.on('finish', async () => {
    ...
    try {
      ...
      // Runs after the HTTP response is already sent, so any Firebase
      // slowness or failure never blocks the user.
      setImmediate(async () => {
        try {
          ...
          // Read current workspace with a 8 s timeout so we never hang forever
          let ws = null;
          try {
            ws = await Promise.race([
              readWorkspaceFromFirebase(userForFirebase),
              new Promise((_, rej) => setTimeout(() => rej(new Error('Firebase read timeout')), 8000))
            ]);
          } catch { ws = null; }
          ...
          // Write with an 8 s timeout
          await Promise.race([
            writeWorkspaceToFirebase(userForFirebase, ws),
            new Promise((_, rej) => setTimeout(() => rej(new Error('Firebase write timeout')), 8000))
          ]);
          // Mirror to localAuth in-memory cache
