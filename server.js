...
// ── Screenshot proxy ──────────────────────────────────────────────────────────
// Fetches a screenshot of a deployed site server-side so the browser never
// hits third-party APIs directly (avoids CORS, auth headers stay server-side).
// Uses puppeteer if available, otherwise falls back to free thum.io service.
// Usage: GET /api/screenshot?url=https://mysite.joytree.site&_n=<nonce>
app.get('/api/screenshot', requireAuth, async (req, res) => {
  ...
  // Try puppeteer first (installed on server), fall back to thum.io proxy
  let puppeteer;
  ...
  if (puppeteer) {
    ...
    try {
      ...
      await new Promise(r => setTimeout(r, 2000)); // let JS paint settle
      ...
      console.warn('[Screenshot] Puppeteer failed, falling back to thum.io:', e.message);
  ...
  const proxyFetch = (url) => new Promise((resolve, reject) => {
    const r = https.get(url, { headers: { 'User-Agent': 'JoytreeBot/1.0' } }, resolve);
    r.on('error', reject);
    r.setTimeout(20000, () => { r.destroy(); reject(new Error('timeout')); });
  });

  // Fallback 1: thum.io — free, no key needed
  try {
    const thumUrl = 'https://image.thum.io/get/width/1280/crop/800/noanimate/allowJPG/'
      + encodeURIComponent(targetUrl) + '?fresh=' + Date.now();
    const upstream = await proxyFetch(thumUrl);
    if (upstream.statusCode === 200) {
      res.set('Content-Type', upstream.headers['content-type'] || 'image/jpeg');
      res.set('Cache-Control', 'public, max-age=60');
      return upstream.pipe(res);
    }
    console.warn('[Screenshot] thum.io returned', upstream.statusCode, '— trying APIFlash');
  } catch (e) {
    console.warn('[Screenshot] thum.io failed:', e.message, '— trying APIFlash');
  }

  // Fallback 2: APIFlash — paid key, most reliable
  try {
    const apiflashUrl = 'https://api.apiflash.com/v1/urltoimage'
      + '?access_key=5ec3b6376bb84deab00221ee16b821f3'
      + '&url=' + encodeURIComponent(targetUrl)
      + '&width=1280&height=800&format=jpeg&quality=90'
      + '&delay=5&fresh=true&response_type=image&_n=' + Date.now();
    const upstream = await proxyFetch(apiflashUrl);
    if (upstream.statusCode === 200) {
      res.set('Content-Type', upstream.headers['content-type'] || 'image/jpeg');
      res.set('Cache-Control', 'public, max-age=60');
      return upstream.pipe(res);
    }
    console.error('[Screenshot] APIFlash returned', upstream.statusCode);
  } catch (e) {
    console.error('[Screenshot] APIFlash failed:', e.message);
  }

  res.status(502).json({ error: 'Could not capture screenshot' });
});
