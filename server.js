...
function ideTermSpawn(sessionId, socket, userId, projectId) {
  ...
  function runCommand(cmd) {
    ...
    // Everything else — run as `bash -c "cmd"` so pipes, redirects, etc. all work
    ...
    child.stdout.on('data', (d) => {
      ...
      try { socket.emit('ide:term:data', d.toString('utf8').replace(/\r?\n/g, '\r\n')); } catch(_){}
    });
    child.stderr.on('data', (d) => {
      lastActivity = Date.now();
      try { socket.emit('ide:term:data', '\x1b[31m' + d.toString('utf8').replace(/\r?\n/g, '\r\n') + '\x1b[0m'); } catch(_){}
    });
    child.on('error', (err) => {
