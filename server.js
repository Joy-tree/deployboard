...
// workspaces (same fetchAllWorkspaces() pattern used elsewhere and
...
async function cloneUserRepo(repoSlug, githubToken, tmpDir, branch) {
  const slug = normalizeRepoSlug(repoSlug);
  const cloneUrl = `https://x-access-token:${githubToken}@github.com/${slug}.git`;
  const branchFlag = branch ? `--branch "${branch}"` : '';
  await execAsync(`git clone --depth=1 ${branchFlag} "${cloneUrl}" "${tmpDir}" 2>&1`, { timeout: 90000 });
}
...
async function createGithubPR({ repoSlug, branchName, githubToken, title, body }) {
  ...
  for (const base of ['main','master','develop']) {
    ...
    // [FIX] GitHub returns 422 "A pull request already exists for ..." when a
    // follow-up run pushes more commits to a branch that already has an open
    // PR. That's not a failure — look the existing PR up and reuse it instead
    // of throwing (which was masking successful pushes as errors).
    const msg = String(err?.message || '').toLowerCase();
    if (msg.includes('already exists')) {
      const existing = await getOpenGithubPRForBranch({ repoSlug, branchName, githubToken });
      if (existing) return existing;
    }
    if (!msg.includes('base')) break;
  }
  throw new Error('GitHub PR creation failed — check repo permissions and branch name.');
}

// [NEW] Look up an already-open PR for a given branch, used both to dedupe
// PR creation and to fetch fresh PR data when explicitly continuing one.
async function getOpenGithubPRForBranch({ repoSlug, branchName, githubToken }) {
  const owner = repoSlug.split('/')[0];
  const headers = { Authorization: `Bearer ${githubToken}`, 'Content-Type': 'application/json', 'User-Agent': 'JoyTree-AI-Agent/1.0' };
  const r = await fetch(`https://api.github.com/repos/${repoSlug}/pulls?state=open&head=${encodeURIComponent(owner)}:${encodeURIComponent(branchName)}`, {
...
async function getGithubPRByNumber({ repoSlug, prNumber, githubToken }) {
  const headers = { Authorization: `Bearer ${githubToken}`, 'Content-Type': 'application/json', 'User-Agent': 'JoyTree-AI-Agent/1.0' };
  const r = await fetch(`https://api.github.com/repos/${repoSlug}/pulls/${prNumber}`, { headers, signal: AbortSignal.timeout(20000) }).catch(() => null);
  if (!r || !r.ok) return null;
...
async function runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext, sourceType, uploadProjectId, uploadSubdomain, userId, userKeys, isContinuation, continueBranch, attachments }) {
  ...
  try {
    ...
    if (isUpload) {
      ...
      // ── GitHub project, CONTINUING prior work: clone the existing branch
      // directly instead of main, so follow-up fixes land on top of the
      // previous commits/PR rather than branching off main's current tip
      // (which was silently discarding earlier work whenever the in-memory
      // session had expired). ──
      push('status', { text: `Continuing on branch "${branchName}"…`, phase: 'clone' });
      try {
        await cloneUserRepo(repoSlug, githubToken, tmpDir, branchName);
      } catch (e) {
        // Branch may have been deleted/merged since — fall back to a fresh
        // branch off main rather than failing the whole run.
        push('status', { text: `Branch "${branchName}" no longer exists — starting a fresh branch off main instead.`, phase: 'clone' });
        await cloneUserRepo(repoSlug, githubToken, tmpDir);
        await execAsync(`cd "${tmpDir}" && git checkout -b "${branchName}" 2>&1`, { timeout: 15000 });
      }
      session.repoCloned = true;
      await execAsync(`cd "${tmpDir}" && git config user.email "ai@joytree.site" && git config user.name "JoyTree AI Agent"`, { timeout: 10000 });
      push('status', { text: `Branch ready. Starting ${getAgentProviderLabel(provider)} agent…`, phase: 'analyze' });
    } else {
...
app.post('/api/ai/agent/start', requireAuth, async (req, res) => {
  const { repoSlug, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext,
          sourceType, uploadProjectId, uploadSubdomain,
  ...
  // [FIX] Reuse the caller-supplied branch (continuing prior work) instead of
  // always minting a brand-new "joytree-ai-fix-<id>" branch off main. Only
  // applies to plain GitHub-source runs — scratch/upload have no concept of
  // a remote branch to continue.
  const resolvedSourceType = isScratch ? 'scratch' : (isUpload ? 'upload' : 'github');
  const isContinuation = resolvedSourceType === 'github' && !!(continueBranch && String(continueBranch).trim());
  const branchName = isContinuation ? String(continueBranch).trim() : `joytree-ai-fix-${sessionId.slice(-10)}`;
  ...
  agentSessions.set(sessionId, {
    ...
    // [FIX] Branch-continuation bookkeeping — see cloneUserRepo call site and
    // finalizeAgentRun for how these get used.
    isContinuation,
    continuePrNumber: continuePrNumber ? Number(continuePrNumber) : null,
