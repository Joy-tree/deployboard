...
async function callDeepSeekChat({ messages, maxTokens = DEEPSEEK_FLOW_MAX_TOKENS, timeoutMs = DEEPSEEK_FLOW_TIMEOUT_MS, temperature = 0.3 } = {}) {
...
async function generateQuestionChunk({ provider = 'groq', topic = '', count = 25, offset = 0, existing = [], maxTokens = 2000 } = {}) {
  ...
  if (provider === 'deepseek') {
    ...
    try {
      ...
      console.error('[DeepSeek chunk] call failed:', e.message);
      throw e;
    }
    const arr = extractChunkArray(text);
    if (!arr || arr.length === 0) {
      console.error('[DeepSeek chunk] unparseable response:', (text || '').slice(0, 300));
      throw new Error('DeepSeek returned unparseable response. Raw: ' + (text || '').slice(0, 120));
    }
    return arr;
  }
...
async function buildFlowWithDeepSeek({ prompt = '', sourceText = '', fileName = '' } = {}) {
  ...
  try {
    const text = await callDeepSeekChat({
      messages: [
        { role: 'system', content: `${AI_FLOW_INSTRUCTION}\nJoytree API v3 runs on DeepSeek high-reasoning AI. If WEB_SEARCH_RESULTS appear in SOURCE_TEXT, use them as current web context.` },
        { role: 'user', content: userInput }
      ],
      temperature: 0.25,
    ...
    const parsed = extractJsonFromText(text);
    if (!parsed) throw new Error('DeepSeek returned non-JSON output. Try rephrasing your prompt.');
    return parsed && typeof parsed === 'object' ? parsed : null;
  } catch (e) {
    if (e.name === 'AbortError') throw new Error('DeepSeek timed out. Try increasing DEEPSEEK_FLOW_TIMEOUT_MS.');
    console.error('[Joytree API v3] buildFlowWithDeepSeek error:', e.message);
    throw e;
  }
}
