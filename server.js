...
const GEMINI_MODEL     = String(process.env.GEMINI_MODEL     || 'gemini-2.0-flash-lite').trim(); // gemini-2.0-flash-lite is free tier; also try gemini-flash-latest
const DEEPSEEK_API_KEY = String(process.env.DEEPSEEK_API_KEY || 'sk-b8d6b6102095481c972ecc34fa7d6fe9').trim();
const DEEPSEEK_MODEL   = String(process.env.DEEPSEEK_MODEL   || 'deepseek-chat').trim(); // deepseek-chat is the stable v3 ID
...
async function callGeminiChat({ messages, maxTokens = 4096, timeoutMs = 90000, temperature = 0.3 } = {}) {
...
async function buildFlowWithV4({ prompt = '', sourceText = '', fileName = '' } = {}) {
  ...
  // Try Gemini first
  try {
    const text = await callGeminiChat({ messages: messages, temperature: 0.25, maxTokens: 8192 });
...
// v4 provider pool with per-provider cooldown tracking
// When a provider hits its quota (429) it cools down for 65s then re-enters the pool
const _v4CooldownUntil = { gemini: 0, deepseek: 0 };
const V4_PROVIDERS = ['gemini', 'deepseek'];

function getAvailableV4Provider() {
  const now = Date.now();
  // Try providers in order; return first one that isn't cooling down
  for (let i = 0; i < V4_PROVIDERS.length; i++) {
    const p = V4_PROVIDERS[i];
    if (now >= _v4CooldownUntil[p]) return p;
  }
  // All cooling — return the one that recovers soonest
  let soonest = V4_PROVIDERS[0];
  for (let i = 1; i < V4_PROVIDERS.length; i++) {
    if (_v4CooldownUntil[V4_PROVIDERS[i]] < _v4CooldownUntil[soonest]) soonest = V4_PROVIDERS[i];
  }
  return soonest;
}
...
function markV4ProviderCooling(provider) {
  ...
  console.warn('[v4] Provider', provider, 'hit quota — cooling for 65s. Other provider takes over.');
}

// Call one chunk using v4 cascade — silently shifts provider on quota hit
async function callV4Chunk(messages) {
  const tried = new Set();
  for (let attempt = 0; attempt < V4_PROVIDERS.length * 2; attempt++) {
    const provider = getAvailableV4Provider();
    tried.add(provider);
    try {
      let text;
      if (provider === 'gemini') {
        text = await callGeminiChat({ messages: messages, temperature: 0.7, maxTokens: 4096 });
      } else {
      ...
      if (isQuota) {
        ...
        console.warn('[v4 chunk] ' + provider + ' quota hit — switching provider silently');
        // Small pause to let the other provider warm up, not a full 65s wait
        await new Promise(function(r){ setTimeout(r, 1000); });
        continue;
      }
      // Non-quota error — try other provider
      console.warn('[v4 chunk] ' + provider + ' error: ' + e.message + ' — trying other provider');
      markV4ProviderCooling(provider); // brief cooldown
      await new Promise(function(r){ setTimeout(r, 500); });
    }
  }
  throw new Error('All v4 providers exhausted for this chunk. Both Gemini and DeepSeek are on cooldown.');
}
