...
async function provisionDbContainer(db) {
  ...
  const memAlloc = db.memory || process.env.DB_DEFAULT_MEMORY || '256m';
  ...
  // Wait for MySQL/MariaDB to finish initializing before returning.
  // docker run returns as soon as the container process starts, but MySQL
  // needs ~20-40s to create the root account, run init scripts, and open
  // the TCP socket. Without this wait, the first query fails with
  // "Can't connect via socket" or "Container is error, not running".
  if (db.engine === 'mysql' || db.engine === 'mariadb') {
    const mysqlBin2 = db.engine === 'mariadb' ? 'mariadb' : 'mysql';
    const maxWaitMs = 90000; // 90 seconds max
    const pollMs    = 3000;  // check every 3 seconds
    const started   = Date.now();
    let ready = false;
    console.log(\`[DB] Waiting for \${db.engine} to initialize on port \${hostPort}…\`);
    while (Date.now() - started < maxWaitMs) {
      await new Promise(r => setTimeout(r, pollMs));
      const ping = await runDocker(
        \`docker exec \${containerName} \${mysqlBin2} -h 127.0.0.1 -u"\${db.user || 'root'}" -p"\${db.pass || ''}" --batch --silent -e "SELECT 1" 2>&1\`,
        5000
      );
      if (ping.ok && (ping.stdout || '').includes('1')) {
        ready = true;
        console.log(\`[DB] \${db.engine} ready after \${Math.round((Date.now()-started)/1000)}s\`);
        break;
      }
      console.log(\`[DB] \${db.engine} not ready yet (\${Math.round((Date.now()-started)/1000)}s)…\`);
    }
    if (!ready) {
      // Log warning but don't throw — container may still come up
      console.warn(\`[DB] \${db.engine} did not respond within \${maxWaitMs/1000}s — proceeding anyway\`);
    }
  }
    ...
    switch (db.engine) {
      ...
      case 'mariadb': {
        ...
        try {
          ...
          cmd = `docker exec ${db.containerName} sh -c '${mysqlBin} -h 127.0.0.1 -u"${db.user||'root'}" -p"${db.pass}" "${db.dbName||'mydb'}" --batch --silent < /tmp/jt_query.sql && rm -f /tmp/jt_query.sql' 2>&1`;
        } finally {
  ...
  try {
    ...
    switch (db.engine) {
      ...
      case 'mariadb': {
        ...
        cmd = `docker exec ${db.containerName} ${mysqlPingBin} -h 127.0.0.1 -u"${db.user || 'root'}" -p"${db.pass || ''}" --batch --silent -e "SELECT 1" 2>&1`;
        break;
      }
      case 'redis':
