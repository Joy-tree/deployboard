...
app.post('/api/auth/github/exchange', async (req, res) => {
  try {
    ...
    res.json({ token, user: { id: user._id || user.id, email: user.email, name: user.name, githubUsername: user.githubUsername, firebaseUid: user.firebaseUid || '' }, firebaseLinked });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
