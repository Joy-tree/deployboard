...
const GEMINI_API_KEY = ''; // Gemini removed — Groq is used exclusively
const GEMINI_MODEL = '';
...
- For quiz/riddle/question/riddle APIs: you MUST populate seedQuestions as a [{question,answer}] array with REAL non-empty content matching the prompt. NEVER leave seedQuestions as an empty array []. Do NOT put questions only inside responseTemplate — always put them in seedQuestions.
...
async function generateQuestionChunk({ provider = 'groq', topic = '', count = 100, offset = 0, existing = [] } = {}) {
  ...
  const callAI = async (apiUrl, apiKey, model, maxTokens, timeoutMs) => {
    ...
    try {
      r = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.7, // higher temp = more variety per chunk
    ...
    if (!r.ok) {
      ...
      throw new Error(`AI HTTP ${r.status}: ${t.slice(0, 200)}`);
    }
    ...
    if (s === -1 || e === -1 || e <= s) throw new Error('AI did not return a JSON array for this chunk.');
    return JSON.parse(text.slice(s, e + 1));
  };

  if (provider === 'xai' && XAI_API_KEY) {
    return callAI('https://api.x.ai/v1/chat/completions', XAI_API_KEY, XAI_MODEL, 8000, 120000);
  }
  if (GROQ_API_KEY) {
    return callAI('https://api.groq.com/openai/v1/chat/completions', GROQ_API_KEY, GROQ_MODEL, 8000, 90000);
  }
...
const CHUNK_SIZE = 80; // safe per-call limit well within token budgets

async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  const allQuestions = [];
  const totalChunks = Math.ceil(total / CHUNK_SIZE);
  const topic = prompt; // full prompt as topic context

  const emitProgress = (done, current, chunkNum) => {
    if (!socketId) return;
    try {
  ...
  emitProgress(0, total, 0);

  for (let chunk = 0; chunk < totalChunks; chunk++) {
    const offset = chunk * CHUNK_SIZE;
    const count = Math.min(CHUNK_SIZE, total - offset);
    let chunkItems = null;
    let lastErr = null;
    ...
    for (const provider of providers) {
      try {
        chunkItems = await generateQuestionChunk({ provider, topic, count, offset, existing: allQuestions });
        if (Array.isArray(chunkItems) && chunkItems.length > 0) break; // success
      } catch (e) {
        lastErr = e;
        console.warn(`[Chunker] chunk ${chunk + 1}/${totalChunks} failed on ${provider}:`, e.message);
        // continue to next provider
      }
    }

    if (!Array.isArray(chunkItems) || chunkItems.length === 0) {
      // If we already have some questions, don't fail — just stop here
      if (allQuestions.length > 0) {
        console.warn(`[Chunker] chunk ${chunk + 1} failed — stopping with ${allQuestions.length} questions collected so far.`);
        break;
      }
      throw new Error(`Failed to generate any questions. Last error: ${lastErr?.message || 'unknown'}`);
    }

    // Normalise and dedup
    const seen = new Set(allQuestions.map(q => q.question.toLowerCase()));
    ...
    emitProgress(allQuestions.length, total, chunk + 1);

    // Small gap between chunks to avoid rate-limit hammering
    if (chunk < totalChunks - 1) await new Promise(r => setTimeout(r, 400));
  }

  emitProgress(allQuestions.length, total, totalChunks);
  return allQuestions;
}
...
async function buildFlowWithGroq({ prompt = '', sourceText = '', fileName = '' } = {}) {
  ...
  try {
    ...
    try {
      r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.3,
          max_tokens: Number(process.env.GROQ_FLOW_MAX_TOKENS || 8000),
          top_p: 1,
          stream: false
        })
      });
    ...
    if (e.name === 'AbortError') throw new Error('Groq timed out while generating a large API payload. Try again or reduce requested output size.');
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  const useChunking = isQuiz && requestedCount > CHUNK_SIZE;
  ...
  if (useChunking) {
    ...
    console.log(`[Chunker] Large request detected: ${requestedCount} items. Splitting into chunks of ${CHUNK_SIZE}.`);
    try {
...
app.post('/api/databases/:id/link', requireAuth, async (req, res) => {
  try {
    ...
    const conn = db.connStr || db.connectionString || '';
    await injectConnStrIntoProject(canonicalProjectId, db.engine, conn, req.user);
    ...
    const keys = envKeyMap[db.engine] || ['DATABASE_URL'];
    const injected = {};
    keys.forEach(k => { injected[k] = conn; });
  ...
  try {
    ...
    switch (db.engine) {
      case 'mongodb':
        ...
        } --eval "db.runCommand({ping:1})" --json=relaxed 2>&1 | tail -1`;
        break;
      case 'postgres':
    ...
    switch (db.engine) {
      ...
      case 'mongodb':  ok = out.includes('"ok"') && (out.includes('"ok" : 1') || out.includes('"ok":1')); break;
