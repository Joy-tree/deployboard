...
app.get('/api/databases', requireAuth, async (req, res) => {
  try {
    ...
    // If MongoDB isn't ready yet, return a 503 so the client knows to keep
    // its existing local cache rather than overwriting it with an empty array.
    if (!isDbReady()) {
      return res.status(503).json({ error: 'db_not_ready', databases: [] });
    }
    let dbs = await Database.find({ ownerUserId: userId }).sort({ createdAt: -1 }).lean();
