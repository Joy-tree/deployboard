...
- CRITICAL: When creating HTML files, NEVER write placeholder stubs like "<h1>Welcome</h1>". Always write a COMPLETE, FUNCTIONAL page with proper DOCTYPE, head (charset, viewport, title, styles), and a fully styled body matching the project's purpose. A newly created HTML file must be deployable and useful immediately — not a skeleton.
- If the error was "missing file", create the file with full real content. If the project is a web app, the HTML must include navigation, content sections, and styling appropriate to the app.
