...
app.use(async (req, res, next) => {
  ...
  try {
    const cd = await CustomDomain.findOne({ domain: host });
    if (!cd) return next();
...
app.get('/api/domains', async (req, res) => {
  try {
    res.json(await CustomDomain.find().sort({ createdAt: -1 }));
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/domains', async (req, res) => {
  ...
  try {
    const existing = await CustomDomain.findOne({ domain: clean });
...
app.post('/api/domains/:domain/verify', async (req, res) => {
  ...
  try {
    ...
    let verified = false;
    try {
      const result = execSync('dig +short CNAME ' + domain, { encoding: 'utf8', timeout: 8000 }).trim();
      if (result.includes('cfargotunnel.com') || result.includes(BASE_DOMAIN)) {
        verified = true;
      }
    } catch(e) {}
    await CustomDomain.findOneAndUpdate({ domain }, { verified });
