...
app.get('/api/domains/check', requireAuth, async (req, res) => {
  try {
    ...
    const availability = Object.fromEntries(availResults);
...
app.post('/api/domains/register', requireAuth, async (req, res) => {
  try {
    ...
    if (!NAMESILO_API_KEY) {
      // Demo mode — persist to Firebase without calling NameSilo
      const ws = (await readWorkspaceFromFirebase(req.user)) || {};
      ws.registeredDomains = Array.isArray(ws.registeredDomains) ? ws.registeredDomains : [];
      const existing = ws.registeredDomains.find(d => d.domain === domain);
      if (!existing) {
        ws.registeredDomains.unshift({
          domain, status: 'active', projectId: projectId || null,
    ...
    // Contact NameSilo to register domain
    ...
    if (code !== 300 && code !== 302) {
      throw new Error(`NameSilo error ${code}: ${regData?.detail || 'Registration failed'}`);
    }

    // Persist to user's Firebase workspace
    const ws = (await readWorkspaceFromFirebase(req.user)) || {};
    ws.registeredDomains = Array.isArray(ws.registeredDomains) ? ws.registeredDomains : [];
    ws.registeredDomains.unshift({
      domain, status: 'active', projectId: projectId || null,
      registeredAt: new Date().toISOString(),
      expiresAt: new Date(Date.now() + 365*24*3600*1000).toISOString(),
      paystackRef, userId, namesiloCode: code
    });
    await writeWorkspaceToFirebase(req.user, ws);

    console.log(`[DomainStore] Registered ${domain} for ${email}`);
...
app.post('/api/paystack/webhook', async (req, res) => {
  try {
    ...
    if (metaType === 'domain_registration' && metaDomain) {
      ...
      return res.json({ ok: true, type: 'domain_registration', domain: metaDomain });
    }
