...
function ideTermSpawn(sessionId, socket, userId, projectId) {
  ...
  function runCommand(cmd) {
    ...
    // Run via `script -q -c "cmd" /dev/null` which allocates a pseudo-TTY.
    // This tricks npm and other tools into streaming output in real time
    // instead of buffering until the process exits.
    const cp = require('child_process');
    let child;
    try {
      const safeCmd = cmd.replace(/"/g, '\\"');
      child = cp.spawn('script', ['-q', '-c', safeCmd, '/dev/null'], {
        cwd,
        env: { ...baseEnv, PWD: cwd, FORCE_COLOR: '1', NPM_CONFIG_PROGRESS: 'true', CI: '' },
