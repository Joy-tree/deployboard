...
/* ═══════════════════════════════════════════════════════════════
   IDE TERMINAL  — Socket.IO + node-pty  (1 GB memory limit)
   Each authenticated user gets one bash session per project.
   The server enforces a 1 GB RSS cap and kills over-limit ptys.
═══════════════════════════════════════════════════════════════ */
const IDE_TERM_MEM_LIMIT_MB  = Number(process.env.IDE_TERM_MEM_LIMIT_MB  || 1024); // 1 GB
const IDE_TERM_MAX_SESSIONS  = Number(process.env.IDE_TERM_MAX_SESSIONS  || 50);   // global cap
const IDE_TERM_RATE_WIN_MS   = Number(process.env.IDE_TERM_RATE_WIN_MS   || 1000);
const IDE_TERM_RATE_MAX_BYTES= Number(process.env.IDE_TERM_RATE_MAX_BYTES|| 65536); // 64 KB/s input

// Try to load optional node-pty; fall back to child_process.spawn if unavailable
let pty;
try { pty = require('node-pty'); } catch (_) { pty = null; }

const ideTermSessions = new Map(); // sessionId → { ptyProc | childProc, socket, userId, bytesThisSec, rateTimer, memTimer }
...
function ideTermSpawn(sessionId, socket, userId, projectId) {
  ...
  // Working directory: user project folder if it exists, else home
  ...
  const env     = { ...process.env, TERM: 'xterm-256color', HOME: process.env.HOME || '/tmp', LANG: 'en_US.UTF-8' };
  ...
  if (pty) {
    try {
      proc  = pty.spawn(shell, [], { name: 'xterm-256color', cols: 80, rows: 24, cwd, env });
      isPty = true;
    } catch (e) {
      socket.emit('ide:term:data', '\r\n\x1b[31mpty spawn failed: ' + e.message + '\x1b[0m\r\n');
    }
  }

  if (!proc) {
    // Fallback: spawn bash with a pipe (limited interactivity)
    const cp = require('child_process');
    proc = cp.spawn(shell, ['--login', '-i'], { cwd, env, stdio: ['pipe','pipe','pipe'] });
    proc.stdout.on('data', d => socket.emit('ide:term:data', d.toString('utf8')));
    proc.stderr.on('data', d => socket.emit('ide:term:data', d.toString('utf8')));
    proc.write = (data) => { try { proc.stdin.write(data); } catch(_){} };
    proc.resize = () => {};
  } else {
    proc.onData(data => socket.emit('ide:term:data', data));
  }

  proc.onExit = proc.onExit || (() => {});
  if (isPty) proc.onExit(({ exitCode }) => {
    socket.emit('ide:term:killed', { reason: 'exit_' + exitCode });
    ideTermCleanup(sessionId);
  });
  if (!isPty) proc.on('exit', (code) => {
  ...
  // Memory polling — check process RSS every 5 s
  const memTimer = setInterval(async () => {
    const pid = proc.pid;
    if (!pid) return;
    try {
      const { execFileSync } = require('child_process');
      // Read RSS from /proc/<pid>/status (Linux)
      const status = fs.readFileSync('/proc/' + pid + '/status', 'utf8');
      const vmrssLine = status.match(/VmRSS:\s+(\d+)\s+kB/);
      const usedKB = vmrssLine ? parseInt(vmrssLine[1], 10) : 0;
  ...
  ideTermSessions.set(sessionId, { proc, isPty, socket, userId, bytesThisSec, rateTimer, memTimer });
...
io.on('connection', (socket) => {
  // ide:term:start — spawn a new shell session
  socket.on('ide:term:start', async (data) => {
    ...
    // Allow if any existing session for this socket; or skip auth for simplicity — auth is via HTTP session
  ...
  socket.on('ide:term:input', (data) => {
    ...
    try { sess.proc.write(input); } catch (_) {}
  });
  ...
  socket.on('ide:term:resize', (data) => {
    ...
    const rows = Math.max(2,  Math.min(200, parseInt(data?.rows) || 24));
    try { sess.proc.resize(cols, rows); } catch (_) {}
  });
  ...
  socket.on('disconnect', () => {
    // Clean up any sessions belonging to this socket
