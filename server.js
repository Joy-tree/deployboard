...
// ── Screenshot proxy ──────────────────────────────────────────────────────────
// Fetches screenshots server-side so the browser never hits third-party APIs
// directly (avoids CORS issues). Chain: thum.io (free) → APIFlash (paid key).
...
app.get('/api/screenshot', requireAuth, async (req, res) => {
  ...
  // Method 1: thum.io — free, no key needed
  try {
  ...
  // Method 2: APIFlash — paid key, most reliable fallback
  try {
