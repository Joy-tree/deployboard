...
app.post('/api/ssh-keys/generate', requireAuth, express.json(), async (req, res) => {
  try {
    ...
    // Use a single base path — ssh-keygen writes <base> (private) and <base>.pub (public)
    const tmpBase = path.join(os.tmpdir(), `jtkey_${Date.now()}`);
    const tmpPriv = tmpBase;
    const tmpPub  = tmpBase + '.pub';

    // Find ssh-keygen — use full path since PATH may not include it on all VPS setups
    let sshKeygenBin = '/usr/bin/ssh-keygen';
    try {
      const { stdout: wp } = await execAsync('which ssh-keygen 2>/dev/null || echo /usr/bin/ssh-keygen');
      sshKeygenBin = wp.trim() || '/usr/bin/ssh-keygen';
    } catch(_) {}
    await execAsync(`"${sshKeygenBin}" -t ed25519 -C "${safeLabel}" -f "${tmpPriv}" -N "" -q`);

    const privateKey = fs.readFileSync(tmpPriv, 'utf8');
    const publicKey  = fs.readFileSync(tmpPub,  'utf8').trim();
    ...
    // Clean up temp files
    try { fs.unlinkSync(tmpPriv); fs.unlinkSync(tmpPub); } catch (_) {}

    // Get fingerprint
    let fingerprint = 'ed25519';
    try {
      const fp = await execAsync(`ssh-keygen -lf "${tmpPub}" 2>/dev/null`).catch(() => null);
      if (fp) fingerprint = fp.stdout.trim();
    } catch (_) {}
    ...
    // Store key record in Firebase RTDB under deployboard_sshkeys/<userKey>/<keyId>
    if (FIREBASE_RTDB_URL && FIREBASE_RTDB_SECRET) {
      ...
      // Don't store private key in Firebase — only metadata
      const { publicKey: _pk, ...meta } = keyRecord;
      await fetch(url, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...meta, publicKey }) }).catch(() => {});
    }
