...
app.post('/api/upload-deploy', requireAuth, async (req, res) => {
  ...
  // Async build
  const buildStart = Date.now();
  const emit = (event, data) => io.emit(event, { deployId: finalDeployId, projectId: cleanSub, source: 'upload', ...data });
...
function ideTermSpawn(sessionId, socket, userId, projectId) {
  ...
  const projDir = projectId ? path.join(SITES_DIR, String(projectId)) : (process.env.HOME || '/tmp');
  const cwd     = fs.existsSync(projDir) ? projDir : (process.env.HOME || '/tmp');
  const shell   = (() => {
    // Find bash wherever it lives on this OS; fall back to sh
  ...
  // Minimal, safe env — don't forward everything from Node's env
  const env = {
  ...
  try {
    ...
    // Use bash -i (interactive) so it emits PS1 prompts even without a PTY.
    // --norc / --noprofile keep startup fast and avoid ~/.bashrc noise.
    const bashArgs = shell.includes('bash') ? ['-i', '--norc', '--noprofile'] : ['-i'];
    proc = cp.spawn(shell, bashArgs, {
      cwd,
      env,
      stdio: ['pipe', 'pipe', 'pipe'],
      detached: false,
    ...
    socket.emit('ide:term:data', '\r\n\x1b[31mFailed to start shell: ' + String(spawnErr.message || spawnErr) + '\x1b[0m\r\n');
    socket.emit('ide:term:killed', { reason: 'spawn_error' });
    return;
  }

  // Safe write helper (proc already has stdin, add unified .write())
  proc.write = (data) => { try { if (proc.stdin && !proc.stdin.destroyed) proc.stdin.write(data); } catch(_){} };
  proc.resize = () => {}; // no-op without pty — clients can call this safely

  // ── Input echo helper ─────────────────────────────────────────────────────
  // Without a PTY, bash does NOT echo typed characters back to stdout.
  // We echo input ourselves so the user sees what they type.
  // We track the current line buffer so we can echo char-by-char including
  // backspace handling, and emit \r\n on Enter so the cursor moves correctly.
  let _echoLine = '';
  function echoInput(raw) {
    ...
    for (const ch of raw) {
      ...
      if (code === 13 || code === 10) {
        // Enter — move to next line; shell will send output + next prompt
        out += '\r\n';
        _echoLine = '';
      } else if (code === 127 || code === 8) {
        // Backspace — erase last char on screen
        if (_echoLine.length > 0) {
          _echoLine = _echoLine.slice(0, -1);
          out += '\b \b'; // move back, overwrite with space, move back again
        }
      } else if (code >= 32) {
        // Printable character
        _echoLine += ch;
      ...
      // Control chars (Ctrl-C etc.) are not echoed visually
    }
  ...
  proc.stderr.on('data', (d) => {
    // Suppress the "can't access tty; job control turned off" warning —
    // it's harmless noise from running sh without a TTY.
    // Also suppress bash's PS0/PS1/PS2 debug noise that some versions print to stderr.
    const txt = d.toString('utf8');
    if (/can't access tty|job control turned off/i.test(txt)) return;
    try { socket.emit('ide:term:data', txt); } catch (_) {}
    lastActivity = Date.now();
  });
  proc.on('error', (err) => {
    try { socket.emit('ide:term:data', '\r\n\x1b[31mShell error: ' + String(err.message) + '\x1b[0m\r\n'); } catch (_) {}
  });
  proc.on('exit', (code) => {
    try {
      socket.emit('ide:term:killed', { reason: 'exit_' + code });
    } catch (_) {}
  ...
  // Per-child RSS guard — reads /proc/<pid>/status every 8 s
  const memTimer = setInterval(() => {
    ...
    try {
      ...
      const usedMB = m ? Math.round(parseInt(m[1], 10) / 1024) : 0;
      socket.emit('ide:term:mem', { usedMB, limitMB: IDE_TERM_MEM_LIMIT_MB });
      if (usedMB > IDE_TERM_MEM_LIMIT_MB) {
        socket.emit('ide:term:data', '\r\n\x1b[31m⚠ Memory limit (' + IDE_TERM_MEM_LIMIT_MB + ' MB) exceeded — session killed\x1b[0m\r\n');
        socket.emit('ide:term:killed', { reason: 'memory_limit' });
        ideTermCleanup(sessionId);
      }
    } catch (_) {}
  }, 8000);

  // Idle timeout — kill shell if no input/output for IDE_TERM_IDLE_TIMEOUT_MS
  ...
  ideTermSessions.set(sessionId, { proc, socket, userId, bytesThisSec, rateTimer, memTimer, idleTimer, echoInput, get lastActivity() { return lastActivity; }, set lastActivity(v) { lastActivity = v; } });
  // Set PS1 and force bash to print the first prompt by sending it to stderr
  // via 'prompt_command'. bash -i will print PS1 to stderr before each command.
  // We send a no-op command (':') so the prompt appears immediately.
  proc.write('export PS1="joytree:\\w# "; :\n');
...
io.on('connection', (socket) => {
  ...
  socket.on('ide:term:input', (data) => {
    try {
      ...
      // Echo the raw keystrokes back so the user sees what they type
      sess.echoInput(input);
      sess.proc.write(input);
    } catch (_) {}
  });
