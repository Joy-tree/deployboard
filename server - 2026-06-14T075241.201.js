...
function proxyResponseOrJsonMismatch(req, res, proxyRes, subdomain) {
  ...
  const clientWantsNoHtml = !acceptsHtml && isHtml;

  if (isHtml && (isProgrammatic || clientWantsNoHtml)) {
    proxyRes.resume();
...
server.on('upgrade', (req, socket, head) => {
  ...
  try {
    // Let Socket.IO handle its own websocket upgrade (dashboard live log stream,
    // build:log/build:step events, etc.). Socket.IO registers its own 'upgrade'
    // listener, but Node calls ALL listeners — if we destroy() the socket here
    // first, Socket.IO never gets to upgrade it, breaking the dashboard's
    // realtime connection (it falls back to/stays on polling and stops
    // receiving build:log events).
