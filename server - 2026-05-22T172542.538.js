...
function errorPage(title, msg) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} — JOYTREE</title>
    <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .card{width:min(94vw,520px);background:#111;border:1px solid rgba(255,255,255,.1);border-radius:20px;overflow:hidden}
    .accent-bar{height:3px;background:#10b981}
    .inner{padding:32px 28px 36px;text-align:center}
    .badge{display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:999px;background:#1a1a1a;border:1px solid rgba(255,255,255,.1);color:#86efac;font-size:.72rem;letter-spacing:.1em;font-weight:800;text-transform:uppercase;margin-bottom:18px}
    .badge::before{content:'';width:7px;height:7px;border-radius:50%;background:#10b981;box-shadow:0 0 8px rgba(16,185,129,.8)}
    h1{font-size:2.1rem;font-weight:900;letter-spacing:-.03em;color:#f8fafc;margin-bottom:14px;line-height:1.1}
    .msg{color:#94a3b8;line-height:1.65;font-size:.95rem;margin-bottom:8px}
    .sub{color:#64748b;font-size:.84rem;line-height:1.55;margin-bottom:24px}
    .divider{height:1px;background:rgba(255,255,255,.07);margin:22px 0}
    a.back-btn{display:inline-flex;align-items:center;gap:8px;padding:13px 28px;border-radius:12px;background:#10b981;color:#fff;text-decoration:none;font-size:.95rem;font-weight:800;transition:background .18s}
    a.back-btn:hover{background:#059669}
    code{background:#1a1a1a;border:1px solid rgba(255,255,255,.08);padding:3px 8px;border-radius:6px;font-family:monospace;font-size:.85em}
    </style></head>
    <body><div class="card">
    ...
    <div class="inner">
      ...
      <h1>${title}</h1>
      <p class="msg">${msg}</p>
      <p class="sub">This subdomain is reserved but no live release has been published yet.</p>
      <div class="divider"></div>
      <a class="back-btn" href="https://${BASE_DOMAIN}">← Back to JOYTREE</a>
    </div>
    </div></body></html>`;
}
