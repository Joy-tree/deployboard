...
- For quiz/riddle/question APIs: include seedQuestions as [{question,answer}] with real content.
- For list/fact/data APIs: include dataSeed as array of relevant objects.
- For any other API: fill responseTemplate with meaningful example output matching the request.
- Never return empty arrays if the prompt asks for content. Populate with real examples.
...
async function buildFlowWithGemini({ prompt = '', sourceText = '', fileBase64 = '', fileMime = '', fileName = '' } = {}) {
  ...
  const parts = [{ text: `${AI_FLOW_INSTRUCTION}\n\n${userInput}` }];
  // Only attach inline_data for mime types Gemini actually supports (images, PDF, audio, video)
  // PPTX/DOCX/XLSX are NOT supported as inline_data — content is passed via sourceText instead
  const GEMINI_SUPPORTED_MIMES = ['image/','application/pdf','audio/','video/'];
  const mimeSupported = GEMINI_SUPPORTED_MIMES.some(m => String(fileMime).startsWith(m));
  if (fileBase64 && fileMime && mimeSupported) {
    parts.push({ inline_data: { mime_type: String(fileMime), data: String(fileBase64).slice(0, 4_000_000) } });
  }
  try {
    // 25s timeout so the request never hangs forever
    ...
    if (!r.ok) {
      ...
      console.error('[Gemini] HTTP error', r.status, msg);
      throw new Error(`Gemini error: ${msg}`);
    }
    const data = await r.json().catch(() => ({}));
    const text = (data?.candidates?.[0]?.content?.parts || []).map(p => p?.text || '').join('\n');
    const parsed = extractJsonFromText(text);
    if (!parsed) throw new Error('Gemini returned non-JSON output. Try rephrasing your prompt.');
    return parsed && typeof parsed === 'object' ? parsed : null;
  } catch (e) {
    if (e.name === 'AbortError') throw new Error('Gemini timed out after 25s. Try a shorter prompt or switch to Groq.');
    console.error('[Gemini] buildFlowWithGemini error:', e.message);
    throw e;
  }
}
...
async function buildFlowWithGroq({ prompt = '', sourceText = '', fileName = '' } = {}) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      const errJson = (() => { try { return JSON.parse(errText); } catch { return null; } })();
      const msg = errJson?.error?.message || errText.slice(0, 200) || `HTTP ${r.status}`;
      console.error('[Groq] HTTP error', r.status, msg);
    ...
    if (e.name === 'AbortError') throw new Error('Groq timed out after 30s. Try a shorter prompt.');
    console.error('[Groq] buildFlowWithGroq error:', e.message);
    throw e;
  }
}

async function buildFlowWithAI({ prompt = '', sourceText = '', fileBase64 = '', fileMime = '', fileName = '', aiProvider = 'auto' } = {}) {
  const useGroq = aiProvider === 'groq' || (aiProvider === 'auto' && !GEMINI_API_KEY && GROQ_API_KEY);
  const useGemini = aiProvider === 'gemini' || (aiProvider === 'auto' && GEMINI_API_KEY);
  let lastError = null;
  if (useGroq && GROQ_API_KEY) {
    try {
      const result = await buildFlowWithGroq({ prompt, sourceText, fileName });
      if (result) return { ...result, _aiProvider: 'groq', _aiModel: GROQ_MODEL };
    } catch (e) { lastError = e; if (aiProvider === 'groq') throw e; }
  }
  if (useGemini && GEMINI_API_KEY) {
    try {
      const result = await buildFlowWithGemini({ prompt, sourceText, fileBase64, fileMime, fileName });
      if (result) return { ...result, _aiProvider: 'gemini', _aiModel: GEMINI_MODEL };
    } catch (e) { lastError = e; if (aiProvider === 'gemini') throw e; }
  }
...
async function buildQuizQuestionsWithGemini({ prompt = '', count = 5 } = {}) {
  if (!GEMINI_API_KEY) return [];
  const instruction = `Return strict JSON only: {"questions":[{"question":"...","answer":"..."}]}.
Generate ${Math.max(1, Math.min(10, Number(count) || 5))} short riddle/quiz questions with exact answers.`;
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent?key=${encodeURIComponent(GEMINI_API_KEY)}`;
  try {
    const r = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ contents: [{ role:'user', parts:[{ text: `${instruction}\n\nPrompt: ${String(prompt || '').slice(0, 4000)}` }] }], generationConfig: { temperature: 0.7, maxOutputTokens: 700 } })
    });
    if (!r.ok) return [];
    const data = await r.json().catch(() => ({}));
    const text = (data?.candidates?.[0]?.content?.parts || []).map(p => p?.text || '').join('\n');
    ...
    return arr.map((q, i) => ({ id: i + 1, question: String(q?.question || '').trim(), answer: String(q?.answer || '').trim() })).filter(q => q.question && q.answer);
  } catch (e) {
    console.error('[Gemini] buildQuizQuestionsWithGemini error:', e.message);
    return [];
  }
}
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  // Hard 55s server-side timeout so the request never hangs forever
  const reqTimeout = setTimeout(() => {
    if (!res.headersSent) res.status(504).json({ error: 'Request timed out after 55s. Try a shorter prompt, use Groq instead, or reduce file size.' });
  }, 55000);
  (async () => {
  const { prompt, routePath = '/quiz', method = 'POST', sourceText = '', fileBase64 = '', fileMime = '', fileName = '', aiProvider = 'auto' } = req.body || {};
  const flowId = `flow_${Date.now()}`;
  if (!GEMINI_API_KEY && !GROQ_API_KEY) return res.status(503).json({ error: 'No AI configured. Set GEMINI_API_KEY or GROQ_API_KEY in server env and restart.' });
  const providerRequested = String(aiProvider || 'auto').toLowerCase();
  if (providerRequested === 'gemini' && !GEMINI_API_KEY) return res.status(503).json({ error: 'Gemini not configured. Set GEMINI_API_KEY in server env.' });
  if (providerRequested === 'groq' && !GROQ_API_KEY) return res.status(503).json({ error: 'Groq not configured. Set GROQ_API_KEY in server env.' });
  // For binary file types (PPTX/DOCX/XLSX), we pass filename+mime as context text since most LLMs can't parse them
  // The frontend should have extracted text where possible; we note the file name for the AI
  ...
  let aiError = null;
  try {
    aiSpec = await buildFlowWithAI({ prompt, sourceText: combinedSourceText, fileBase64, fileMime, fileName, aiProvider: providerRequested });
  } catch (e) {
    aiError = e.message || String(e);
  }
  if (!aiSpec) return res.status(502).json({ error: aiError || 'AI did not return a valid flow spec. Try rephrasing your prompt or switching AI provider.' });
  if (!aiSpec.responseTemplate || typeof aiSpec.responseTemplate !== 'object') return res.status(502).json({ error: 'AI returned invalid output template. Try rephrasing your prompt.' });
  const usedProvider = aiSpec._aiProvider || providerRequested;
  const usedModel = aiSpec._aiModel || (usedProvider === 'groq' ? GROQ_MODEL : GEMINI_MODEL);
  ...
  if (isQuiz) {
    ...
    const questions = aiQuestions.slice(0, 50).map((q, i) => ({ id:i+1, question:String(q?.question||'').trim(), answer:String(q?.answer||'').trim() })).filter(q => q.question && q.answer);
    if (!questions.length) return res.status(502).json({ error: 'AI returned no quiz/riddle content. Refine your prompt (e.g. "generate 5 riddle questions with answers").' });
    getCollection(flowId, 'quiz_questions').push(...questions);
    saveVirtualDb();
  }
  const aiDataSeed = Array.isArray(aiSpec?.dataSeed) ? aiSpec.dataSeed.slice(0, 100) : [];
...
app.get('/api/developer/ai/status', requireAuth, (req, res) => {
  res.json({
    ok: true,
    geminiConfigured: !!GEMINI_API_KEY, geminiModel: GEMINI_MODEL,
    groqConfigured: !!GROQ_API_KEY, groqModel: GROQ_MODEL,
...
async function injectConnStrIntoProject(projectId, engine, connStr, user = null) {
  ...
  try {
    ...
    if (isDbReady() && mongoose.Types.ObjectId.isValid(String(projectId))) {
      ...
      if (user) {
        const patch = {};
        keys.forEach(k => { patch[k] = connStr; });
        updateLocalWorkspaceProject(user, String(projectId), { envVars: { ...(user?.workspace?.projects||[]).find(p=>String(p.id||p._id||'')===String(projectId))?.envVars, ...patch } });
      }
      return;
    }
  ...
  try {
    ...
    switch (db.engine) {
      case 'mongodb': {
        ...
        const mongoArgs = JSON.stringify(mongoCmd).replace(/'/g, "'\\''");
        cmd = `docker exec ${db.containerName} mongosh --quiet ${db.user ? `--username "${db.user}" --password "${db.pass}" --authenticationDatabase admin` : ''} ${db.dbName||'mydb'} --eval "JSON.stringify(db.runCommand(${JSON.stringify(mongoCmd)}))"`;
        break;
      }
      case 'postgres': {
      ...
      case 'redis': {
        ...
        cmd = `docker exec ${db.containerName} redis-cli ${db.pass ? `-a "${db.pass}"` : ''} ${redisArgs}`;
        break;
      }
      default:
