...
async function runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext, sourceType, uploadProjectId, userId }) {
  ...
  try {
    ...
    if (isUpload) {
      ...
      const projDir  = path.join(UPLOADS_DIR, userId, uploadProjectId);
      let filesDir   = path.join(projDir, 'files');
      // [FIX] If the project was uploaded but never extracted (files/ missing),
      // extract the stored archive now so the agent has full file access.
