...
const CHUNK_SIZE = 25;        // Groq / xAI — 25 items per call stays well within rate limits
const CHUNK_SIZE_LOCAL = 10;  // llama.cpp on CPU — Qwen 3B does ~15 tok/s, 10 items ≈ 200 tokens ≈ 15s per chunk

async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  const allQuestions = [];
  const isLocal = aiVersion === 'v3';
  const chunkSize = isLocal ? CHUNK_SIZE_LOCAL : CHUNK_SIZE;
  const totalChunks = Math.ceil(total / chunkSize);
  const topic = prompt;

  const emitProgress = (done, chunkNum) => {
    if (!socketId) return;
    try {
      const pct = total > 0 ? Math.round((done / total) * 100) : 0;
      const etaNote = isLocal && done < total
        ? ` (~${Math.round(((total - done) / chunkSize) * 18)}s left)`
        : '';
      io.to(socketId).emit('ai:chunk:progress', {
        done, total, chunkNum, totalChunks,
        message: done >= total
          ? `✓ All ${total} items generated!`
          : `Generating batch ${chunkNum}/${totalChunks} — ${done} of ${total} done${etaNote}…`
      });
    } catch (_) {}
  };
  ...
  for (let chunk = 0; chunk < totalChunks; chunk++) {
    ...
    // v3 = local llama.cpp only; v2 = Groq then xAI; v1 = Groq only
    const providers = isLocal ? ['local'] : (aiVersion === 'v2' && XAI_API_KEY ? ['groq', 'xai'] : ['groq']);
    ...
    // Inter-chunk gap: none for local (it's slow enough already), 1.5s for Groq rate limits
    if (!isLocal && chunk < totalChunks - 1) await new Promise(r => setTimeout(r, 1500));
  }
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  const effectiveChunkSize = initialAiVersion === 'v3' ? CHUNK_SIZE_LOCAL : CHUNK_SIZE;
  // For v3: always chunk if count > local chunk size; for v1/v2: chunk if count > standard chunk size
