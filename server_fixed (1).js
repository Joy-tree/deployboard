...
app.post('/api/upload-project', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // ── Sync metadata to Firebase BEFORE responding so the client can trust it persisted ──
    ...
    try {
      ...
      const wrote = await writeWorkspaceToFirebase(userForFirebase, ws);
      firebaseSynced = !!wrote;
      if (!firebaseSynced) fbSyncError = 'firebase_write_returned_false';
      // Also keep local auth in sync
      ...
      fbSyncError = String(fbErr?.message || 'firebase_sync_failed').slice(0, 200);
      console.warn('[upload-project] Firebase sync failed:', fbSyncError);
    }

    // ── Respond to the client with sync status so the frontend knows what happened ──
    res.json({
      ok: true, projectId,
      lang: result.lang, fileCount: result.fileCount,
      ...
      serverPath: path.join(UPLOADS_DIR, userId, projectId, 'files'),
      firebaseSynced,
      firebaseSyncError: fbSyncError || undefined
    });
  } catch(e) {
...
app.get('/api/upload-projects', requireAuth, async (req, res) => {
  try {
    ...
    // Always return ok:true with the list (even if empty) so the client can
    // distinguish "Firebase returned empty list" from "Firebase call failed"
    return res.json({ ok: true, projects: uploads, firebaseKey: !!(userForFirebase?.email) });
  } catch(e) {
    return res.status(500).json({ error: e.message });
  }
});
