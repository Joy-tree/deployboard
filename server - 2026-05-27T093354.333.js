    ...
    try {
      r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.3,
          max_completion_tokens: Number(process.env.GROQ_FLOW_MAX_TOKENS || 8000),
          top_p: 1,
          stream: false
        })
      });
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  if (isQuiz) {
    const rawQuestions = useChunking ? chunkedQuestions : (Array.isArray(aiSpec?.seedQuestions) ? aiSpec.seedQuestions : []);
