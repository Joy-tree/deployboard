...
function probeHttp(host, port, timeoutMs = 1000) {
  ...
  return new Promise((resolve, reject) => {
    const req = http.request({ host, port, path: '/', method: 'GET', timeout: timeoutMs }, (res) => {
      // Reject pure HTML responses: a foreign service (e.g. the Joytree server
      // itself, or another container's error page) can be reachable on the seed
      // port and return HTTP 200/404 with text/html content. Accepting that would
      // register the wrong port and cause the "<!DOCTYPE ... is not valid JSON"
      // error the user sees in the browser. We require a non-HTML content type
      // (application/json, text/plain, application/javascript, etc.) OR we allow
      // any response if the app itself answers on a port it actually bound to
      // (that distinction is handled by the caller via discoveredSet).
      // An absence of Content-Type (many plain Node servers on '/') is fine.
