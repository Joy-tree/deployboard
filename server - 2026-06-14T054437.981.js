...
app.post('/api/deploy', requireAuth, async (req, res) => {
  ...
  // Emit to BOTH the user private room AND the deploy-specific room.
  // The deploy room ensures logs arrive even when auth:join races with socket connect,
  // or when ownerUserId is blank/temp (Firebase-only mode with u_<timestamp> IDs).
  const emit = (event, data) => {
    ...
    if (_ownerRoomId) {
      io.to('user:' + _ownerRoomId).emit(event, payload);
    } else {
...
io.on('connection', socket => {
  ...
  socket.on('auth:join', ({ userId } = {}) => {
    if (userId) {
      ...
      console.log('[Socket.io] auth:join — socket', socket.id, 'joined room user:' + userId);
    }
  });
  // Client can subscribe to a specific deployment's log stream by deployId.
  // This is the fallback for when the user room join fails (race condition,
  // ownerUserId mismatch, Firebase-only mode with temp IDs, etc.).
  socket.on('subscribe:deploy', ({ deployId } = {}) => {
    if (deployId) {
      socket.join('deploy:' + String(deployId));
      socket._joytreeDeployId = String(deployId);
      console.log('[Socket.io] subscribe:deploy — socket', socket.id, 'joined room deploy:' + deployId);
    }
  });
