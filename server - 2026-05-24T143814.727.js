...
function containerExists(name) {
  const r = runDocker(`docker inspect --format '{{.State.Status}}' ${name} 2>/dev/null || echo ''`);
  return r.stdout && r.stdout !== '';
}

// Get container running state
function containerStatus(name) {
  const r = runDocker(`docker inspect --format '{{.State.Status}}' ${name} 2>/dev/null || echo 'missing'`);
...
app.get('/api/databases', requireAuth, async (req, res) => {
  try {
    ...
    let dbs = [];
    if (isDbReady()) {
      dbs = await Database.find({ ownerUserId: userId }).sort({ createdAt: -1 }).lean();
      // Sync live Docker status for each
      dbs = dbs.map(db => {
        if (db.containerName) {
          const liveStatus = containerStatus(db.containerName);
          if (liveStatus !== db.status) {
            Database.updateOne({ _id: db._id }, { $set: { status: liveStatus } }).catch(() => {});
    ...
    res.json({ databases: dbs.map(d => ({ ...d, id: String(d._id), connectionString: d.connStr || d.connectionString || '' })) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        if (isDbReady()) {
          await Database.updateOne({ _id: dbRecord._id }, {
            $set: { status: 'error', errorMessage: String(e.message).slice(0, 300), updatedAt: new Date() }
          });
        }
      }
    });
...
app.post('/api/databases/:id/start', requireAuth, async (req, res) => {
  try {
    ...
    db.status = 'running'; db.updatedAt = new Date(); await db.save();
...
app.post('/api/databases/:id/stop', requireAuth, async (req, res) => {
  try {
    ...
    db.status = 'stopped'; db.updatedAt = new Date(); await db.save();
...
app.post('/api/databases/:id/delete', requireAuth, async (req, res) => {
  try {
    ...
    if (db.volume && db.volume.startsWith('/var/joytree-data/')) {
