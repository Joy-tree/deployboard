...
async function generateQuestionChunk({ provider = 'groq', topic = '', count = 100, offset = 0, existing = [] } = {}) {
  const existingSample = existing.slice(-10).map(q => q.question).join('\n');
  const userMsg = `Topic/context: ${topic}\n\nGenerate EXACTLY ${count} items (items ${offset + 1} to ${offset + count}).\n\n${existingSample ? `EXISTING (do not repeat):\n${existingSample}\n` : ''}Return ONLY a valid JSON array of {question, answer} objects. No explanation, no markdown.`;
    ...
    try {
      r = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.7,
          max_tokens: maxTokens,   // ← always max_tokens, never max_completion_tokens
          top_p: 1,
          stream: false
        })
      });
  ...
  if (provider === 'cerebras') {
    ...
    const s = text.indexOf('[');
    const e = text.lastIndexOf(']');
    if (s === -1 || e === -1 || e <= s) throw new Error('Cerebras did not return a JSON array for this chunk.');
    return JSON.parse(text.slice(s, e + 1));
  }
...
const CHUNK_SIZE = 25;          // Groq / xAI / Cerebras — 25 items per call
const CHUNK_SIZE_CEREBRAS = 25; // Cerebras is fast; same chunk size as cloud providers

// Fun status messages cycled while waiting between chunks so users aren't bored
...
const INTER_CHUNK_DELAY_MS = 60000; // 60 seconds between chunks for v1/v2 to respect rate limits
...
async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  ...
  const emitProgress = (done, chunkNum, message) => {
    ...
    try {
      ...
      io.to(socketId).emit('ai:chunk:progress', {
        done, total, chunkNum, totalChunks,
  ...
  for (let chunk = 0; chunk < totalChunks; chunk++) {
    ...
    // v3 = Cerebras; v2 = Groq then xAI; v1 = Groq only
    const providers = aiVersion === 'v3' ? ['cerebras'] : (aiVersion === 'v2' && XAI_API_KEY ? ['groq', 'xai'] : ['groq']);

    for (const provider of providers) {
      try {
        chunkItems = await generateQuestionChunk({ provider, topic, count, offset, existing: allQuestions });
        if (Array.isArray(chunkItems) && chunkItems.length > 0) break;
      } catch (e) {
        lastErr = e;
    ...
    if (!Array.isArray(chunkItems) || chunkItems.length === 0) {
      if (allQuestions.length > 0) {
        console.warn(`[Chunker] chunk ${chunk + 1} failed — returning ${allQuestions.length} items collected so far.`);
        break;
      }
      throw new Error(`Failed to generate items. Last error: ${lastErr?.message || 'unknown'}`);
    }
    ...
    // Inter-chunk gap: 60s for v1/v2 to respect rate limits; Cerebras (v3) is fast so only 2s
    if (chunk < totalChunks - 1) {
      const gapMs = aiVersion === 'v3' ? 2000 : INTER_CHUNK_DELAY_MS;
      const gapSteps = Math.ceil(gapMs / 3000);
      for (let step = 0; step < gapSteps; step++) {
        const waitMsg = CHUNK_WAIT_MESSAGES[waitMsgIdx % CHUNK_WAIT_MESSAGES.length];
        waitMsgIdx++;
        const secsLeft = Math.round((gapMs - step * 3000) / 1000);
        emitProgress(allQuestions.length, chunk + 1, `${waitMsg} (next batch in ${secsLeft}s — ${allQuestions.length}/${total} done)`);
        await new Promise(r => setTimeout(r, Math.min(3000, gapMs - step * 3000)));
      }
    }
  }

  emitProgress(allQuestions.length, totalChunks);
  return allQuestions;
}
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  const routeTimeoutMs = 300000; // 300s for all versions (Cerebras is fast enough)
  ...
  const usedModel = aiSpec?._aiModel || (selectedAiVersion === 'v3' ? 'Joytree API v3 (Cerebras)' : (selectedAiVersion === 'v2' ? 'Joy AI v2' : 'Joy AI v1'));
