...
function getAgentProvider(preferredProvider) {
  if (preferredProvider === 'claude'  && ANTHROPIC_API_KEY) return 'claude';
  if (preferredProvider === 'groq'    && GROQ_API_KEY)      return 'groq';
  if (preferredProvider === 'xai'     && XAI_API_KEY)       return 'xai';
...
async function callAgentLLM({ provider, messages, tools, temperature = 0.2, maxTokens = 8000 }) {
  ...
  if (provider === 'claude') {
    ...
    if (!r.ok) {
      ...
      if (r.status === 429) throw new Error('Anthropic API error 429: Rate limit reached and still throttled after 3 retries. Wait a moment and try again, or check your usage tier limits in the Anthropic Console.');
      throw new Error(`Anthropic API error ${r.status}: ${e?.error?.message || r.statusText}`);
    }

    const data = await r.json();

    // Convert Anthropic response → OpenAI-like format so our agentic loop works uniformly
  ...
  const apiKey = provider === 'xai' ? XAI_API_KEY : GROQ_API_KEY;
  const model  = provider === 'xai' ? XAI_MODEL   : GROQ_MODEL;
  ...
  const doCall = (tokens) => fetch(apiUrl, {
    ...
    signal: AbortSignal.timeout(provider === 'xai' ? 90000 : 45000)
  });
  ...
  if (!r.ok) {
    ...
    if (r.status === 429) throw new Error(`${provider.toUpperCase()} API error 429: Rate limit reached and still throttled after 3 retries. Your ${provider === 'xai' ? 'xAI' : 'Groq'} free tier has a per-minute token limit (TPM). Wait a minute and try again, switch to a model with a higher TPM, or upgrade your ${provider.toUpperCase()} plan. Raw: ${e.slice(0, 200)}`);
    if (r.status === 413) throw new Error(`${provider.toUpperCase()} API error 413: This conversation has grown too large for ${model}'s rate-limit tier. Try a model with a higher TPM limit, or upgrade your ${provider.toUpperCase()} plan. Raw: ${e.slice(0, 200)}`);
...
// ── Main agent runner ─────────────────────────────────────────────
async function runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext }) {
  // [FIX] Normalize once up front — the frontend may pass a full GitHub URL.
  // Every downstream use (clone, push remote, PR API) now gets a clean
  // "owner/repo" slug instead of a doubled-host or malformed path.
  ...
  // Resolve provider
  const provider = getAgentProvider(preferredProvider);
  if (!provider) {
  ...
  try {
    ...
    while (iteration < MAX_ITERATIONS) {
      ...
      const groqData = await callAgentLLM({
        provider,
        messages,
    ...
    if (hasChanges) {
      ...
      await execAsync(
        `cd "${tmpDir}" && git add -A && git commit -m "${commitMsg}" 2>&1`,
        { timeout: 20000 }
      );
      ...
      const prData = await createGithubPR({
        ...
        body: [
          ...
          '### Error',
          '```',
          (errorText || 'See deployment logs').slice(0, 1500),
          '```',
          '',
          '### What Changed',
          finalSummary.slice(0, 2000),
          '',
          '### Diff Stats',
          '```',
          diffStat.trim(),
      ...
      session.prUrl    = prData.html_url;
      session.prNumber = prData.number;
      session.status   = 'done';
    ...
    session.status = 'error';
    push('error', { message: e.message || String(e) });
  } finally {
    ...
    if (tmpDir && tmpDir.startsWith(path.join(os.tmpdir(), 'joytree-agent-'))) {
...
app.get('/api/ai/agent/providers', requireAuth, (req, res) => {
  res.json({
    providers: [
      { id: 'claude', label: `Claude (${ANTHROPIC_MODEL})`, available: !!ANTHROPIC_API_KEY, best: true,  description: 'Best for complex code reasoning and precise edits' },
      { id: 'groq',   label: `Llama (${GROQ_MODEL})`,       available: !!GROQ_API_KEY,      best: false, description: 'Fast open-source model, great for most fixes' },
      { id: 'xai',    label: `Grok (${XAI_MODEL})`,         available: !!XAI_API_KEY,       best: false, description: 'xAI Grok, strong reasoning capabilities' }
    ],
    recommended: getAgentProvider(null)
  });
});

// POST /api/ai/agent/start
app.post('/api/ai/agent/start', requireAuth, async (req, res) => {
  const { repoSlug, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext } = req.body || {};
  const githubToken = req.user?.githubAccessToken || '';

  const provider = getAgentProvider(preferredProvider);
  if (!provider)    return res.status(503).json({ error: 'No AI provider configured. Add ANTHROPIC_API_KEY, GROQ_API_KEY, or XAI_API_KEY to your .env and restart.' });
  if (!githubToken) return res.status(400).json({ error: 'No GitHub token — please sign in with GitHub to use the AI Agent.' });
  if (!repoSlug)    return res.status(400).json({ error: 'repoSlug is required (e.g. username/my-app)' });
  ...
  runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext }).catch(e => {
