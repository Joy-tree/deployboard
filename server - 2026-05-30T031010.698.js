...
// ── Multipart upload ─────────────────────────────────────────────────────────
// We use busboy (already bundled with many Node versions) or fall back to a
// streaming approach that works without multer to keep zero extra deps.
app.post('/api/upload-project', requireAuth, async (req, res) => {
  ...
  // Stream via busboy
  let busboy;
  try { busboy = new (require('busboy'))({ headers: req.headers, limits: { fileSize: 260 * 1024 * 1024 } }); }
  catch(e) {
    try { busboy = require('busboy')({ headers: req.headers, limits: { fileSize: 260 * 1024 * 1024 } }); }
    catch(e2) { return res.status(500).json({ error: 'busboy not available: ' + e2.message }); }
  }
  ...
  busboy.on('field', (name, val) => {
    if (name === 'projectId') projectId = String(val).trim();
    if (name === 'projectName') projectName = String(val).trim().slice(0, 80);
  });

  busboy.on('file', (fieldname, fileStream, info) => {
    const filename = typeof info === 'object' ? (info.filename || 'archive') : String(info || 'archive');
    origFilename = path.basename(filename);
    if (!projectId) projectId = 'upload_' + Date.now();
    const userUploadDir = path.join(UPLOADS_DIR, userId, projectId);
    try { fs.mkdirSync(userUploadDir, { recursive: true }); } catch(e) {}
    savedArchivePath = path.join(userUploadDir, origFilename);
    const ws = fs.createWriteStream(savedArchivePath);
    fileStream.pipe(ws);
    ws.on('error', e => console.error('[upload] write error:', e));
  });

  busboy.on('finish', async () => {
    if (!savedArchivePath || !fs.existsSync(savedArchivePath)) {
      return res.status(400).json({ error: 'No file received' });
    }
    // Extract archive
    try {
      const result = await extractUploadedArchive(savedArchivePath, path.join(UPLOADS_DIR, userId, projectId, 'files'));
      return res.json({
        ok: true, projectId,
  ...
  busboy.on('error', e => res.status(500).json({ error: 'Upload error: ' + e.message }));
  req.pipe(busboy);
});
...
app.post('/api/upload-deploy', requireAuth, async (req, res) => {
  ...
  // Upsert project record — use a synthetic repoUrl so existing code works
  const uploadRepoUrl = `upload://${cleanSub}`;
  let project;
  try {
    project = await Project.findOneAndUpdate(
      { subdomain: cleanSub },
      { name, subdomain: cleanSub, repoUrl: uploadRepoUrl,
        branch: 'upload',
        installCmd: installCmd || 'npm install',
        buildCmd: buildCmd || '',
        startCmd: startCmd || '',
        ...
        nodeVer: nodeVer || '20',
        siteType: siteType || 'static',
        appPort, billingPlan: planKey,
        memoryLimit: runtimeProfile.memoryLimit,
        cpuShares: runtimeProfile.cpuShares,
        memorySwap: runtimeProfile.memorySwap,
        envVars: envVars || {},
        isDockerfileDeploy: false, isWorker: false,
        ownerUserId: String(req.user?._id || req.user?.id || ''),
    ...
    project = {
      _id: 'local_' + Date.now(), name, subdomain: cleanSub, repoUrl: uploadRepoUrl,
      branch: 'upload', installCmd: installCmd||'npm install', buildCmd: buildCmd||'',
      startCmd: startCmd||'', outputDir: outputDir||'dist', nodeVer: nodeVer||'20',
      siteType: siteType||'static', appPort, billingPlan: planKey,
      memoryLimit: runtimeProfile.memoryLimit, cpuShares: runtimeProfile.cpuShares, memorySwap: runtimeProfile.memorySwap,
      envVars: envVars||{}, isDockerfileDeploy:false, isWorker:false,
      uploadProjectId: projectId, uploadFilesDir: filesDir,
      save: async () => {}
    };
  }
  ...
  try {
    deployment = await new Deployment({
      projectId: project._id, projectName: name, branch: 'upload',
      status: 'pending', source: 'upload', triggerSha: ''
    }).save();
  } catch(e) {
    deployment = {
      _id: 'local_' + Date.now(), projectId: project._id, projectName: name,
      branch: 'upload', status: 'pending', source: 'upload', triggerSha: '', logs: [],
      startedAt: new Date(), save: async () => {}
    };
  }

  const deployId = deployment._id.toString();
  deployStopRequests.delete(deployId);
  res.json({ ok: true, deployId, projectId: String(project._id), message: 'Upload deployment started',
             source: 'upload', liveUrl: `https://${cleanSub}.${BASE_DOMAIN}` });

  // Async build from uploaded files
  const buildStart = Date.now();
  deployment.status = 'building';
  try { await deployment.save(); } catch(e) {}

  const emit = (event, data) => io.emit(event, { deployId, projectId: String(project._id), source: 'upload', ...data });
  ...
  (async () => {
    try {
      ...
      // Run the buildRunner with filesDir as the source instead of cloning
      ...
      try { await deployment.save(); } catch(e) {}
      try { await Project.findByIdAndUpdate(project._id, { lastDeployedAt: new Date(), status: 'live' }); } catch(e) {}
      ...
      emit('build:log', { line: `\n\x1b[32m✓ Deployed in ${duration}s\x1b[0m` });
      emit('build:done', { status: 'success', duration, liveUrl: cf?.url || `https://${cleanSub}.${BASE_DOMAIN}` });
    } catch(e) {
      const safeErr = String(e?.message || 'unknown error').slice(0, 400);
      const duration = Math.round((Date.now() - buildStart) / 1000);
      deployment.status = 'failed'; deployment.endedAt = new Date(); deployment.duration = duration;
      deployment.error = safeErr;
      try { await deployment.save(); } catch(_e) {}
      try { await Project.findByIdAndUpdate(project._id, { status: 'error' }); } catch(_e) {}
      emit('build:log', { line: `\x1b[31m[Joytree]\x1b[0m Build failed: ${safeErr}` });
...
async function extractUploadedArchive(archivePath, destDir) {
  ...
  const MAX_FILE_SIZE = 500 * 1024; // 500 KB per file for storage
