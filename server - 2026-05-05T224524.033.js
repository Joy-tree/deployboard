...
app.post('/api/projects/:id/autodeploy', requireAuth, async (req, res) => {
  try {
    ...
    if (!isDbReady()) {
      if (!GLOBAL_WEBHOOK_SECRET) return res.status(503).json({ error: 'MongoDB unavailable and GLOBAL_WEBHOOK_SECRET not configured' });
      return res.json({ ok:true, enabled: !!enabled, webhookUrl: `${req.protocol}://${req.get('host')}/api/github/webhook`, secret: GLOBAL_WEBHOOK_SECRET, mode: 'global' });
    }
...
app.post('/api/github/webhook', async (req, res) => {
  try {
    if (!GLOBAL_WEBHOOK_SECRET) return res.status(400).json({ error: 'GLOBAL_WEBHOOK_SECRET not configured' });
    ...
    const all = await Project.find({ autoDeployEnabled: true }).lean().maxTimeMS(6000).catch(()=>[]);
    let queued = 0;
    for (const p of all) {
