...
async function runBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog, isDockerfileDeploy, isWorker }) {
  ...
  if (isPython) {
    return runPythonBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog });
  }
...
async function runPythonBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  log(`\x1b[90m[python] Streaming logs in structured batches\x1b[0m`);
  const buildCmd = (project.buildCmd || '').trim() || runtime.buildCmd;
  if (buildCmd && buildCmd !== 'echo skip') {
    log(`\x1b[90m$ ${buildCmd}\x1b[0m`);
    try {
      await runPythonCommandInContainer({ projectRoot, pythonImage, envObj: env, command: buildCmd, log });
    } catch (e) {
      if (/collectstatic.*CommandError|STATIC_ROOT|does not exist/i.test(String(e.message || ''))) {
        log(`\x1b[33m[Joytree]\x1b[0m collectstatic failed — skipping (STATIC_ROOT may not be configured). Set STATIC_ROOT in env vars if needed.`);
      } else if (/No migrations to apply|already applied/i.test(String(e.message || ''))) {
        log(`\x1b[33m[Joytree]\x1b[0m Migrations: already up to date — continuing.`);
      } else {
        throw e;
      }
    }
  ...
  try {
    ...
    fs.mkdirSync(appDir, { recursive: true });
    const excludes = ['.git', '__pycache__', '*.pyc', '.env'];
    const copyFiltered = (src, dst) => {
      fs.mkdirSync(dst, { recursive: true });
      for (const entry of fs.readdirSync(src)) {
        if (excludes.includes(entry) || entry.endsWith('.pyc')) continue;
  ...
  // Map app dir to Docker-reachable path
  ...
  try { await exec('docker', ['pull', pythonImage], {}, log); } catch(_) { log(`\x1b[33m[docker] Using cached image\x1b[0m`); }
  ...
  const runArgs = [
    ...
    pythonImage,
    'sh', '-c',
    // Re-install deps at runtime since pip installs to the image, not the volume
    `pip install -r requirements.txt -q 2>/dev/null || pip install -r requirements.txt --quiet; ${startCmd}`
  ];
  ...
  if (!stable) {
    ...
    throw new Error(`Python container exited during startup. Check logs above.\nCommon fixes:\n  • Ensure requirements.txt exists in your repo root\n  • Check your start command (e.g. gunicorn myapp.wsgi:application)\n  • Set DJANGO_SETTINGS_MODULE in Environment Variables`);
  }
