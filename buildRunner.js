...
async function runServerBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  // Wait for app to boot (up to 30s)
  ...
  try {
    ...
    log(`\x1b[31m[docker] Could not get container IP: ${e.message}\x1b[0m`);
  }

  // Detect actual port the app is listening on by scanning common ports
  const portsToCheck = [appPort, 3000, 8080, 8000, 5000, 4000, 3001];
  let actualPort = 0;

  if (containerIP) {
    for (const p of portsToCheck) {
      const open = await checkTcpPort(containerIP, p, 2000);
      if (open) {
        actualPort = p;
        log(`\x1b[32m[docker] ✓ App is listening on port ${p}\x1b[0m`);
        break;
      }
    }
  }
  ...
  if (!actualPort) {
    // App not responding — show logs
    log(`\x1b[31m[docker] App not responding on any port — showing container logs:\x1b[0m`);
    log(`\x1b[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\x1b[0m`);
    try { await exec('docker', ['logs', '--tail', '40', containerName], {}, log); } catch(e) {}
    log(`\x1b[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\x1b[0m`);
  } else {
