...
// ── Main agent runner ─────────────────────────────────────────────
async function runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext, sourceType, uploadProjectId, userId }) {
  ...
  try {
    ...
    if (isUpload) {
      ...
      const filesDir = path.join(UPLOADS_DIR, userId, uploadProjectId, 'files');
      if (!fs.existsSync(filesDir)) {
        push('error', { message: 'Could not find your uploaded project files on the server. Try re-uploading the project, then run the fix again.' });
        session.status = 'error';
        return;
      }
      // Copy the stored files into the sandbox (exclude heavy dirs)
    ...
    if (isUpload) {
      // ── Upload project: detect changes, then zip the fixed project for download ──
      push('status', { text: 'Packaging your fixed project…', phase: 'commit' });
      // The agent edited files in-place in tmpDir. Build a quick change list by
      // comparing against the original stored files.
      const origDir = path.join(UPLOADS_DIR, userId, uploadProjectId, 'files');
      let changedCount = 0;
      try {
        const { stdout: diffOut } = await execAsync(
          `diff -rq "${origDir}" "${tmpDir}" 2>/dev/null | head -200`, { timeout: 30000 }
        ).catch(() => ({ stdout: '' }));
      ...
      if (hasChanges) {
        // Zip the fixed project so the user can download + re-upload it.
        const zipDir = path.join(UPLOADS_DIR, userId, '_ai_fixes');
        fs.mkdirSync(zipDir, { recursive: true });
        const zipName = `joytree-ai-fix-${(projectName || uploadProjectId || 'project').replace(/[^a-zA-Z0-9_-]/g,'_')}-${sessionId.slice(-6)}.zip`;
        const zipPath = path.join(zipDir, zipName);
        // Zip the contents of tmpDir (exclude VCS/build junk)
        await execAsync(
          `cd "${tmpDir}" && zip -r -q "${zipPath}" . -x '*.git*' -x '*/node_modules/*' -x '*/dist/*' 2>&1`,
          { timeout: 120000 }
        );
        session.zipPath = zipPath;
        session.zipName = zipName;
        session.status  = 'done';
        const downloadUrl = `/api/ai/agent/download/${sessionId}`;
        push('fix_zip_ready', {
          downloadUrl, zipName,
          summary: finalSummary,
          subdomain: session.uploadProjectId || '',
...
app.post('/api/ai/agent/start', requireAuth, async (req, res) => {
  ...
  runAgentSession(sessionId, { repoSlug, githubToken, errorText, deploymentId, projectName, userPrompt, mode, preferredProvider, analysisContext,
                               sourceType: isUpload ? 'upload' : 'github', uploadProjectId: uploadProjectId || uploadSubdomain || '', userId }).catch(e => {
