...
async function runBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog, isDockerfileDeploy, isWorker }) {
  ...
  // If "static" is set → always static.
  // If "server" is set → always server.
  // If blank → attempt server build which auto-detects from package.json after
  //   clone; it falls back to static internally if no start script is found.
  ...
  // siteType is blank AND no startCmd — try server first (it will fall back to
  // static internally if the repo has no start script after clone + inspect).
  return runServerBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog });
}
...
async function runDockerfileBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  const runArgs = [
    ...
    // Bind container port to a localhost port so the host-side proxy can reach it
    '-p', `127.0.0.1:${appPort}:${exposedPort}`,
    '-e', `PORT=${exposedPort}`,
  ...
  // Use appPort (host-side -p binding), not targetPort (container-internal).
  registry[project.subdomain] = `127.0.0.1:${appPort}`;
  fs.writeFileSync(portsFile, JSON.stringify(registry, null, 2));

  log(`\x1b[32m[docker] ✓ Dockerfile app is live at ${project.subdomain} → 127.0.0.1:${appPort} (container internal: ${targetPort})\x1b[0m`);
...
async function runPythonBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  // Stop the previous stable container BEFORE starting the candidate so
  // the port binding is free. If the candidate fails, restore the old one.
  const _pythonPausedName = await pauseStableForPortRelease(containerName, log);

  let _pythonDockerOk = false;
  try {
    await exec('docker', runArgs, {}, log);
    _pythonDockerOk = true;
  } catch(dockerErr) {
    // Restore previous container if docker run itself failed
    await restorePausedContainer(_pythonPausedName, containerName, log);
    throw dockerErr;
  }
  ...
  try {
    ...
    log(`\x1b[32m[docker] ✓ Proxy registered: ${project.subdomain} → 127.0.0.1:${appPort} (container internal: ${targetPort})\x1b[0m`);
    await archivePreviousContainer(_pythonPausedName, project.subdomain, log);
    await exec('docker', ['rename', candidateContainerName, containerName], {}, () => {});
...
async function runGenericBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog,
  ...
  const runArgs = [
    ...
    // Bind to localhost so host-side proxy can reach the container
    '-p', `127.0.0.1:${expectedPort}:${expectedPort}`,
    '-e', `PORT=${expectedPort}`,
  ...
  // Stop the previous stable container BEFORE starting the candidate to free the port.
  const _genericPausedName = await pauseStableForPortRelease(containerName, log);

  try {
    await exec('docker', runArgs, {}, log);
  } catch(dockerErr) {
    await restorePausedContainer(_genericPausedName, containerName, log);
    throw dockerErr;
  }
  log(`\x1b[32m[docker] ✓ Container started\x1b[0m`);
  ...
  try {
    ...
    registry[project.subdomain] = `127.0.0.1:${appPort}`;
    fs.writeFileSync(pFile, JSON.stringify(registry, null, 2));
    log(`\x1b[32m[docker] ✓ Proxy: ${project.subdomain} → 127.0.0.1:${appPort} (container internal: ${livePort})\x1b[0m`);
    await archivePreviousContainer(_genericPausedName, project.subdomain, log);
    await exec('docker', ['rename', candidateContainerName, containerName], {}, () => {});
    await cleanupArchivedContainers(project.subdomain, DEPLOY_HISTORY_KEEP, log);
  } catch (e) {
...
async function runServerBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  const dockerArgs = [
    ...
    '--pids-limit',   PIDS_LIMIT,
    '-m',             runtime.memory,
    // Bind container port to localhost so the host-side reverse proxy can reach it.
    // Without this, the container port is only reachable on the Docker bridge network
    // (container names don't resolve from the host process).
    '-p',             `127.0.0.1:${appPort}:${expectedPort}`,
    '-e',             `PORT=${expectedPort}`,
  ...
  // Stop previous stable container BEFORE starting candidate to free the port.
  const _serverPausedName = await pauseStableForPortRelease(containerName, log);

  try {
    await exec('docker', dockerArgs, {}, log);
  } catch(dockerErr) {
    await restorePausedContainer(_serverPausedName, containerName, log);
    throw dockerErr;
  }
  log(`\x1b[32m[docker] ✓ Container started\x1b[0m`);
  ...
  try {
    ...
    const targetPort = normalizePort(livePort, expectedPort);
    // The host-side port binding is always appPort (from -p 127.0.0.1:appPort:expectedPort).
    // targetPort is the container-internal port; we must use appPort for the proxy registry
    // so the Cloudflare tunnel hits the correct host-bound port.
    // If the app used a different port than expectedPort, we'd need to re-run docker with
    // the correct -p binding — but for now we trust appPort as the host binding.
    registry[project.subdomain] = `127.0.0.1:${appPort}`; // host-bound port, NOT container-internal
    fs.writeFileSync(pFile, JSON.stringify(registry, null, 2));
    log(`\x1b[32m[docker] ✓ Proxy registered: ${project.subdomain} → 127.0.0.1:${appPort} (container internal: ${targetPort})\x1b[0m`);
    if (targetPort !== expectedPort) {
      log(`\x1b[33m[docker] App is listening on port ${targetPort} internally (expected ${expectedPort}) — host binding is ${appPort}\x1b[0m`);
    }
    ...
    await archivePreviousContainer(_serverPausedName, project.subdomain, log);
    await exec('docker', ['rename', candidateContainerName, containerName], {}, () => {});
...
// Stop the currently running stable container to free its port binding BEFORE
// starting the candidate. Renames it to a temp "paused" name so it can be
// restored quickly on failure, or fully archived on success.
// Returns the paused container name (or null if nothing was running).
async function pauseStableForPortRelease(containerName, log) {
  const state = await getContainerState(containerName).catch(() => '');
  if (!state) return null;
  const paused = `${containerName}-pausing-${Date.now()}`;
  try {
    log(`\x1b[90m[docker] Stopping previous container to free port…\x1b[0m`);
    await exec('docker', ['stop', '-t', '15', containerName], {}, () => {});
    await exec('docker', ['rename', containerName, paused], {}, () => {});
    log(`\x1b[90m[docker] Previous container paused as ${paused}\x1b[0m`);
    return paused;
  } catch(e) {
    log(`\x1b[33m[docker] Could not pause previous container: ${e.message} — continuing\x1b[0m`);
    // Force-remove the old container so the port is definitely free
    try { await exec('docker', ['rm', '-f', containerName], {}, () => {}); } catch(_) {}
    return null;
  }
}

// Restore the paused container if candidate failed (fast rollback)
async function restorePausedContainer(pausedName, containerName, log) {
  if (!pausedName) return;
  try {
    await exec('docker', ['rename', pausedName, containerName], {}, () => {});
    await exec('docker', ['start', containerName], {}, () => {});
    log(`\x1b[33m[docker] Previous release restored after candidate failure\x1b[0m`);
  } catch(e) {
    log(`\x1b[31m[docker] Could not restore previous container: ${e.message}\x1b[0m`);
  }
}

async function archivePreviousContainer(pausedName, subdomain, log) {
  // pausedName is the temp "pausing-NNN" name returned by pauseStableForPortRelease.
  // If null (nothing was running before), nothing to archive.
  if (!pausedName) return;
  const archived = pausedName.replace('-pausing-', '-prev-');
  try {
    await exec('docker', ['rename', pausedName, archived], {}, () => {});
    log(`\x1b[90m[docker] Archived previous release: ${archived}\x1b[0m`);
  } catch(_) {
    // Already gone or rename failed — not critical
  }
}
...
async function cloneRepo(project, buildDir, githubToken, log) {
  ...
  // Skip git clone entirely and just confirm the dir is populated.
  if (project.repoUrl === '__UPLOAD__' && project._uploadBuildDir) {
    const fileCount = countFiles(buildDir);
    if (fileCount === 0) {
      // buildDir was wiped and re-created by the builder before calling us — re-copy
      copyDir(project._uploadBuildDir, buildDir);
      log(`[90m[upload] Restored ${countFiles(buildDir)} files from upload cache[0m`);
    } else {
      log(`[90m[upload] Using ${fileCount} pre-loaded files (skipping git clone)[0m`);
    }
    return;
  }
...
// decide: if there's a package.json with a "start" script, OR a known server
// entry file (server.js, app.js, index.js with http/express usage, etc.),
...
function autoDetectUploadServerApp(uploadFilesDir, startCmdHint, log) {
  // 1. Explicit start command always wins
  if (startCmdHint) return true;

  // 2. Find the project root (deepest dir with package.json)
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      // Has an explicit start script → server app
      if (scripts.start && String(scripts.start).trim()) {
        log(`[36m[Joytree][0m Auto-detected server app: package.json has "start" script: "${scripts.start}"`);
        return true;
      }

      // Has dependencies that are server frameworks → server app
      ...
      if (matchedFramework) {
        log(`[36m[Joytree][0m Auto-detected server app: found server framework dependency: "${matchedFramework}"`);
        return true;
      }

      // Has a main field pointing to a .js file that likely starts a server
      ...
      if (mainFile && fs.existsSync(mainFile)) {
        ...
        if (/require\(['"]express['"]\)|require\(['"]http['"]\)|\.listen\s*\(|createServer\s*\(|new\s+Server\s*\(/.test(src)) {
          log(`[36m[Joytree][0m Auto-detected server app: main file (${pkg.main}) contains server code`);
          return true;
        }
      }
    } catch(_) {}
  ...
  // 3. Check common server entry filenames for server patterns
  ...
  for (const fname of candidateFiles) {
    ...
    try {
      ...
      if (/require\(['"]express['"]\)|require\(['"]fastify['"]\)|require\(['"]koa['"]\)|require\(['"]http['"]\)|require\(['"]https['"]\)|\.listen\s*\(|createServer\s*\(|new\s+Server\s*\(|import\s+express|import\s+fastify|import\s+Koa/.test(src)) {
        log(`[36m[Joytree][0m Auto-detected server app: ${fname} contains server/listen code`);
        return true;
      }
    } catch(_) {}
  }
...
async function runUploadBuild({ deployId, project, sitesDir, tmpDir, appPort, emit, onLog, uploadFilesDir }) {
  ...
  // Java, Rust, .NET, Elixir, Bun, Deno), route them to a full framework build
  // that spins up a Docker container — NOT the static build which requires HTML.
  const isPython = runtime.startsWith('python') || siteType === 'python';
  const isGo     = runtime.startsWith('go');
  const isPHP    = runtime.startsWith('php');
  const isRuby   = runtime.startsWith('ruby');
  ...
  // For framework runtimes: copy uploaded files into a temp buildDir then run
  // the same framework builder used for GitHub deploys (they work on a dir, not a git clone).
  async function routeToFrameworkBuild(builderFn) {
    // Framework builders clone into tmpDir/deployId — for uploads we seed that
    // dir from the uploaded files so the builder finds the project files there.
    const buildDir = path.join(tmpDir, deployId);
    if (fs.existsSync(buildDir)) fs.rmSync(buildDir, { recursive: true, force: true });
    fs.mkdirSync(buildDir, { recursive: true });
    log(`[90m[upload] Copying ${countFiles(uploadFilesDir)} files to build workspace…[0m`);
    copyDir(uploadFilesDir, buildDir);
    log(`[32m[upload][0m Files ready`);
    // Patch cloneRepo so the builder skips git clone and uses our pre-seeded dir
    const origProject = { ...project, _uploadBuildDir: buildDir, repoUrl: '__UPLOAD__' };
    return builderFn({ deployId, project: origProject, sitesDir, tmpDir, githubToken: null, appPort, emit, onLog });
  }
  ...
  // siteType is static → always static.
  // siteType is server → always server (Node.js Docker container).
  // siteType is blank  → auto-detect from the uploaded files.
  ...
  if (!isServerApp && siteType !== 'static') {
    ...
    if (isServerApp) {
      log(`[33m[Joytree][0m ℹ Tip: set Site Type = "Server App" in project settings to skip auto-detection next time.`);
    }
  }
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (scripts.start) {
          ...
          log(`[32m[Joytree][0m ✓ Using start script from package.json: "${scripts.start}"`);
        } else if (pkg.main) {
          project = { ...project, startCmd: `node ${pkg.main}` };
          log(`[32m[Joytree][0m ✓ Using main entry: "node ${pkg.main}"`);
        } else {
          ...
          if (found) {
            ...
            log(`[32m[Joytree][0m ✓ Auto-resolved start command: "node ${found}"`);
          }
        }
      } catch(_) {}
    }
...
async function runUploadStaticBuild({ deployId, project, sitesDir, tmpDir, emit, onLog, uploadFilesDir, log, cleanSub }) {
  ...
  if (!hasStaticEntry) {
    ...
    if (!hasStaticEntry) {
      log(`\x1b[31m[error]\x1b[0m No HTML entry file found in your upload.`);
      log(`\x1b[33m[hint]\x1b[0m If this is a backend/server app (Node.js, PHP, Python, Go, Ruby, etc.):`);
      log(`\x1b[33m[hint]\x1b[0m   → Set the Runtime (PHP, Python, Go…) in Project Settings so the right builder runs`);
      log(`\x1b[33m[hint]\x1b[0m   → Or set Site Type = "Server App" for Node.js backends`);
      log(`\x1b[33m[hint]\x1b[0m If this is a static site: make sure your upload contains at least one .html file`);
      throw new Error('Upload deploy failed: no HTML entry file found. If this is a backend app, set the Runtime (PHP/Python/Go/etc.) or Site Type = "Server App" in project settings.');
    }
  }
...
async function runUploadServerBuild({ deployId, project, sitesDir, tmpDir, appPort, emit, onLog, uploadFilesDir, log, cleanSub }) {
  ...
  // Pause (stop+rename) previous stable container to free port before starting candidate.
  // This allows rollback if the new container fails, unlike rm -f which destroys it.
  const _uploadServerPausedName = await pauseStableForPortRelease(containerName, log);

  const uploadMemory = (project.memoryLimit || '512m').toString();
  const runArgs = [
    'run', '-d', '--restart=unless-stopped',
    '--name', candidateContainerName,
    '--network', networkName,
    ...
    '--pids-limit', PIDS_LIMIT,
    '-m', uploadMemory,
    // Bind to localhost so host proxy can reach it — critical for server apps
    '-p', `127.0.0.1:${appPort}:${expectedPort}`,
    '-e', `PORT=${expectedPort}`,
    '-e', `NODE_ENV=production`,
    ...Object.entries(envObj).flatMap(([k,v]) => ['-e', `${k}=${v}`]),
  ...
  // Archive the paused previous container, then promote candidate to stable name
  await archivePreviousContainer(_uploadServerPausedName, cleanSub, log);
  await exec('docker', ['rename', candidateContainerName, containerName], {}, () => {});
  await cleanupArchivedContainers(cleanSub, DEPLOY_HISTORY_KEEP, log);

  // Register port — always use appPort (host-bound) not targetPort (container-internal)
  try {
    const portsFile = path.join(sitesDir, 'ports.json');
    let registry = {};
    try { registry = JSON.parse(fs.readFileSync(portsFile,'utf8')); } catch(e) {}
    registry[cleanSub] = `127.0.0.1:${appPort}`;
    fs.writeFileSync(portsFile, JSON.stringify(registry, null, 2));
  } catch(e) {}
  log(`\x1b[32m[docker]\x1b[0m Container running on 127.0.0.1:${appPort} (container internal: ${targetPort})`);
  emitStep(emit, 'start', 'done');
