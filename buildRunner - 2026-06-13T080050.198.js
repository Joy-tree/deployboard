...
async function runGenericBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog,
  ...
  const runArgs = [
    ...
    dockerImage, 'sh', '-c', `export PATH=/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:$PATH && ${resolvedStartCmd}`
  ];
...
async function runServerBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  // Wrap the start command with an explicit PATH so that npm/node/yarn/pnpm
  // are always resolvable even when Docker runs the entrypoint via a minimal
  // /bin/sh that doesn't include /usr/local/bin (where node images install them).
  // Without this, `npm start` exits 127 ("not found") on node:*-bullseye/alpine.
  const wrappedStartCmd = `export PATH=/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:$PATH && ${resolvedStartCmd}`;
  dockerArgs.push(nodeImage, 'sh', '-c', wrappedStartCmd);
  ...
  try {
    ...
    if (targetPort !== expectedPort) {
      log(`\x1b[33m[docker] ⚠ App did not bind PORT=${expectedPort} — it started on ${targetPort} instead.\x1b[0m`);
      log(`\x1b[33m[docker]   Routing proxy to ${targetPort}. To silence this warning, make your app read process.env.PORT.\x1b[0m`);
    }
...
async function detectLivePort(containerName, preferredPort, startupTimeoutSeconds, log) {
  const candidates = [];
  // Track which ports came from real discovery vs. seed guesses so we can
  // prefer them during probing (discovered ports are inserted at the front).
  ...
  const addDiscovered = (v) => {
    ...
    if (!candidates.includes(n)) {
      candidates.unshift(n); // discovered ports go to the FRONT
      discoveredSet.add(n);
    }
  };
  const addSeed = (v) => {
    const n = Number(v);
    if (!Number.isInteger(n) || n <= 0) return;
    if (!candidates.includes(n)) candidates.push(n); // seed ports go to the BACK
  };

  // ── Pre-scan: discover real listeners before building the seed list ──────
  // This runs before we even start the loop so that if the app is already
  // up (e.g. fast Node startup), we try its real port first and never
  // accidentally promote an unrelated service on the seed port.
  let fallbackIP = '';
  ...
  try {
    const earlyDiscovered = await detectListeningPorts(containerName);
    const earlyLogPort    = await detectPortFromContainerLogs(containerName);
    if (earlyLogPort) addDiscovered(earlyLogPort);
    earlyDiscovered.forEach(addDiscovered);
    if (earlyDiscovered.length) {
      log(`\x1b[90m[docker] Discovered listening ports (pre-scan): ${earlyDiscovered.join(', ')}\x1b[0m`);
    }
  } catch (_) {}

  // Seed ports come AFTER discovered ones so we only fall back to them when
  // nothing the app actually bound to answers HTTP.
  ...
  for (let attempt = 1; attempt <= startupTimeoutSeconds; attempt++) {
    if (attempt === 1 || attempt % 5 === 0) {
      try {
        ...
        // Newly discovered ports go to the front of the candidate list.
    ...
    // Check logs on every attempt — a log-announced port is more reliable than
    // a raw TCP listener (which may be a non-HTTP internal service).
    try {
      const logPort = await detectPortFromContainerLogs(containerName);
      if (logPort) addDiscovered(logPort);
    } catch (_) {}
    for (const port of candidates) {
      // Allow HTML responses from ports the app actually bound to (full-stack apps
      // serve HTML). Only reject HTML from seed/fallback ports where a foreign
      // service might be answering.
      ...
      try {
        ...
        // Fall back to direct container IP HTTP probing before giving up.
        if (fallbackIP) {
          try {
            ...
            // Log first-time probe failures for discovered ports so we can see WHY they fail.
            if (allowHtml && attempt <= 3) {
              log(`\x1b[90m[docker] probe ${port} failed — hostname: ${httpErr1 && httpErr1.message}, IP: ${err2 && err2.message}\x1b[0m`);
            }
          }
        } else if (allowHtml && attempt <= 3) {
          log(`\x1b[90m[docker] probe ${port} failed — ${httpErr1 && httpErr1.message}\x1b[0m`);
        }
      }
    }
...
function probeHttp(host, port, timeoutMs = 1000, allowHtml = false) {
  const http = require('http');
  return new Promise((resolve, reject) => {
    const req = http.request({ host, port, path: '/', method: 'GET', timeout: timeoutMs }, (res) => {
      // For discovered ports (ports the app actually bound to), we allow any HTTP
      // response including HTML — the app IS a full-stack server that serves HTML.
      // For seed/fallback ports only, we reject HTML responses to avoid accidentally
      // registering a foreign service (e.g. another container's error page) that
      // returns text/html. An absence of Content-Type is always fine.
      if (allowHtml) {
        // Drain the body so the socket closes cleanly, then resolve.
        res.resume();
      ...
      const isHtmlOnly = ct.startsWith('text/html') && !ct.includes('application/');
      // Also sniff the first chunk for an HTML doctype just in case CT is missing.
      let body = '';
  ...
  return new Promise((resolve, reject) => {
    ...
    if (BUILD_TOOL_COMMANDS.has(cmd) && !isDockerLifecycleCmd && options.lowPriority !== false) {
      if (_NICE_AVAILABLE && _IONICE_AVAILABLE) {
        spawnCmd  = 'nice';
        spawnArgs = ['-n', '15', 'ionice', '-c2', '-n7', '--', cmd, ...args];
      } else if (_NICE_AVAILABLE) {
        spawnCmd  = 'nice';
        spawnArgs = ['-n', '15', '--', cmd, ...args];
      } else if (_IONICE_AVAILABLE) {
        spawnCmd  = 'ionice';
        spawnArgs = ['-c2', '-n7', '--', cmd, ...args];
      }
