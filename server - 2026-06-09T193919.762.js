...
app.get('/api/domains/transfer', async (req, res) => {
  ...
  try {
    ...
    let project = null;
    if (isMongoReady()) {
      project = await Project.findOne({ subdomain }).lean().maxTimeMS(5000);
    }
    if (!project) {
      done(false, `No deployed project found with subdomain "${subdomain}". Deploy it first.`);
      return;
    }
    send('step', `✓ Project "${project.name || subdomain}" found (${project.status || 'deployed'})`);
    await new Promise(r => setTimeout(r, 150));

    // 2. Check domain isn't already in use by a different project
    send('log', 'Checking for existing domain conflicts…');
    let existingEntry = null;
    if (isMongoReady()) {
      existingEntry = await CustomDomain.findOne({ domain }).lean().maxTimeMS(5000);
    } else {
      existingEntry = memDomains.find(d => d.domain === domain) || null;
    }

    if (existingEntry && existingEntry.subdomain !== subdomain) {
      done(false, `Domain "${domain}" is already assigned to project "${existingEntry.subdomain}". Remove it first.`);
      return;
    }
    if (existingEntry && existingEntry.subdomain === subdomain) {
    ...
    // 4. Save/upsert the domain mapping
    send('log', 'Saving domain → project mapping to database…');
    let savedEntry = null;
    if (!isMongoReady()) {
      if (!existingEntry) {
        const entry = { domain, subdomain, verified: dnsResult.verified, createdAt: new Date() };
        memDomains.push(entry);
        savedEntry = entry;
      } else {
        existingEntry.subdomain = subdomain;
        existingEntry.verified = dnsResult.verified;
        savedEntry = existingEntry;
      }
      send('warn', '⚠ MongoDB unavailable — saved in memory (resets on restart)');
    } else {
      savedEntry = await CustomDomain.findOneAndUpdate(
        { domain },
