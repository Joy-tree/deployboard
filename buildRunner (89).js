...
async function runPythonBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog }) {
  ...
  // Resolve install command: user override > auto-detected requirements file > pyproject/setup fallback
  let installCmd = (project.installCmd || '').trim();
  if (!installCmd || installCmd === 'pip install -r requirements.txt') {
    installCmd = reqFile ? `pip install -r ${reqFile}` : resolvePythonInstallFallback(projectRoot, log);
  }
  ...
  try {
    ...
    // Graceful: if requirements file is missing or empty, warn and continue like Node does
    if (/No such file or directory|Could not open requirements|not found/i.test(String(e.message || ''))) {
      log(`\x1b[33m[Joytree] Warning: Could not install dependencies — ${e.message.split('\n')[0]}\x1b[0m`);
      log(`\x1b[33m[Joytree] Continuing without pre-installed packages. Add a requirements.txt to your repo to fix this.\x1b[0m`);
    } else {
      throw e;
    }
  }
  ...
  log(`\x1b[90m[python] Streaming logs in structured batches\x1b[0m`);
  let buildCmd = (project.buildCmd || '').trim() || runtime.buildCmd;

  // Auto-detect obvious build steps the user may have forgotten to configure
  if (!buildCmd || buildCmd === 'echo skip') {
    buildCmd = autoDetectPythonBuildCmd(projectRoot, runtime.framework, log);
  }

  if (buildCmd && buildCmd !== 'echo skip') {
    log(`\x1b[90m$ ${buildCmd}\x1b[0m`);
    try {
      await runPythonCommandInContainer({ projectRoot, pythonImage, envObj: env, command: buildCmd, log });
    } catch (e) {
      const msg = String(e.message || '');
      if (/collectstatic.*CommandError|STATIC_ROOT|does not exist/i.test(msg)) {
        log(`\x1b[33m[Joytree] collectstatic failed — STATIC_ROOT not configured. Skipping. Set STATIC_ROOT in Env Variables.\x1b[0m`);
      } else if (/No migrations to apply|already applied|up to date/i.test(msg)) {
        log(`\x1b[33m[Joytree] Migrations: already up to date — continuing.\x1b[0m`);
      } else if (/Table.*already exists|relation.*already exists/i.test(msg)) {
        log(`\x1b[33m[Joytree] Migration tables already exist — continuing.\x1b[0m`);
      } else if (/manage\.py.*not found|No module named/i.test(msg)) {
        log(`\x1b[33m[Joytree] Build step skipped — ${msg.split('\n')[0]}\x1b[0m`);
      } else {
        log(`\x1b[33m[Joytree] Build step warning: ${msg.split('\n')[0]} — continuing deployment.\x1b[0m`);
      }
    }
  } else {
    log(`\x1b[90m(no build step detected — skipping)\x1b[0m`);
  }
  emitStep(emit, 'build', 'done');
  ...
  const startCmd = (project.startCmd || '').trim() || runtime.startCmd(expectedPort);
  ...
  const runtimeReqInstall = reqFile
    ? `pip install -r ${reqFile} -q 2>&1 | tail -3`
    : `pip install -r requirements.txt -q 2>/dev/null || pip install -r requirements.dev.txt -q 2>/dev/null || true`;
...
// Auto-detects obvious build steps the user may have forgotten to set.
// Mirrors how Node.js auto-detects "npm run build" when build script exists in package.json.
function autoDetectPythonBuildCmd(projectRoot, framework, log) {
  const hasManage = fs.existsSync(path.join(projectRoot, 'manage.py'));
  const isDjango  = framework === 'Django' || hasManage;

  if (isDjango && hasManage) {
    log(`\x1b[33m[Joytree] Auto-detected Django manage.py — running migrations automatically.\x1b[0m`);
    return 'python manage.py migrate --noinput 2>&1 || true';
  }
  return 'echo skip';
}
...
async function runGenericBuild({ deployId, project, sitesDir, tmpDir, githubToken, appPort, emit, onLog,
  dockerImage, findRoot, extraEnv = {}, stepLabel }) {
  ...
  const installCmd = (project.installCmd || '').trim();
  const buildCmd   = (project.buildCmd   || '').trim();
  const startCmd   = (project.startCmd   || '').trim();
...
async function runGoBuild(opts) {
  ...
  const installCmd = opts.project.installCmd || 'go mod download';
  const buildCmd   = opts.project.buildCmd   || 'go build -o server .';
  const startCmd   = opts.project.startCmd   || `PORT=${normalizePort(opts.appPort,8080)} ./server`;
  return runGenericBuild({ ...opts, dockerImage: image, stepLabel: 'go',
    findRoot: (dir, log) => { const f = findFileRoot(dir, ['go.mod'], log); return f || dir; },
...
async function runPhpBuild(opts) {
  ...
  const image  = `php:${phpVer}-cli`;
  // Need composer — use a composer image for install, then php for runtime
  const runtime  = String(opts.project.runtime || '').toLowerCase();
  const isLaravel = runtime === 'php-laravel';
  const isSymfony = runtime === 'php-symfony';
  const installCmd = opts.project.installCmd || 'composer install --no-dev --optimize-autoloader';
  const buildCmd   = opts.project.buildCmd   || (isLaravel ? 'php artisan optimize && php artisan migrate --force' : isSymfony ? 'php bin/console cache:clear --env=prod' : 'echo skip');
  const port       = normalizePort(opts.appPort, 8080);
  const startCmd   = opts.project.startCmd   || (isLaravel ? `php artisan serve --host=0.0.0.0 --port=${port}` : `php -S 0.0.0.0:${port} -t public/`);
  // Use composer image which includes PHP for build
  const buildImage = 'composer:2';
  const runImage   = `php:${phpVer}-cli`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: buildImage, stepLabel: 'php',
...
async function runRubyBuild(opts) {
  ...
  const runtime = String(opts.project.runtime || '').toLowerCase();
  const isRails = runtime === 'ruby-rails';
  const port    = normalizePort(opts.appPort, 3000);
  const installCmd = opts.project.installCmd || 'bundle install';
  const buildCmd   = opts.project.buildCmd   || (isRails ? 'bundle exec rails assets:precompile && bundle exec rails db:migrate' : 'echo skip');
  const startCmd   = opts.project.startCmd   || (isRails ? `bundle exec puma -C config/puma.rb -b tcp://0.0.0.0:${port}` : `bundle exec rackup -o 0.0.0.0 -p ${port}`);
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'ruby',
...
// ── JVM (Java / Kotlin / Quarkus) ────────────────────────────────────────────
async function runJvmBuild(opts) {
  const javaVer = String(opts.project.javaVer || '17');
  const image   = `eclipse-temurin:${javaVer}-jdk-jammy`;
  const runtime = String(opts.project.runtime || '').toLowerCase();
  const port    = normalizePort(opts.appPort, 8080);
  const isMaven  = fs.existsSync(path.join(opts.tmpDir, opts.deployId, 'pom.xml')) || runtime.includes('quarkus');
  const isGradle = !isMaven;
  const installCmd = opts.project.installCmd || (isMaven ? './mvnw dependency:resolve -q 2>/dev/null || mvn dependency:resolve -q' : './gradlew dependencies -q 2>/dev/null || gradle dependencies -q');
  const buildCmd   = opts.project.buildCmd   || (isMaven ? './mvnw package -DskipTests -q 2>/dev/null || mvn package -DskipTests -q' : './gradlew bootJar -q 2>/dev/null || gradle bootJar -q');
  const startCmd   = opts.project.startCmd   || `java -jar \$(find . -name "*.jar" -path "*/target/*" -o -name "*.jar" -path "*/build/libs/*" | head -1) --server.port=${port}`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'java',
...
async function runRustBuild(opts) {
  ...
  const installCmd = opts.project.installCmd || 'echo skip';
  const buildCmd   = opts.project.buildCmd   || 'cargo build --release';
  const startCmd   = opts.project.startCmd   || `PORT=${port} \$(find ./target/release -maxdepth 1 -type f -executable ! -name "*.d" | head -1)`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'rust',
...
async function runDotnetBuild(opts) {
  ...
  const port      = normalizePort(opts.appPort, 8080);
  const installCmd = opts.project.installCmd || 'dotnet restore';
  const buildCmd   = opts.project.buildCmd   || 'dotnet publish -c Release -o out --no-restore';
  const startCmd   = opts.project.startCmd   || `ASPNETCORE_URLS=http://0.0.0.0:${port} dotnet out/*.dll`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'dotnet',
    findRoot: (dir, log) => findFileRoot(dir, ['*.csproj', '*.sln', 'Program.cs'], log, true) || dir,
    extraEnv: { DOTNET_ENVIRONMENT: 'Production', ASPNETCORE_ENVIRONMENT: 'Production' }
  });
}
...
async function runElixirBuild(opts) {
  ...
  const installCmd = opts.project.installCmd || 'mix local.hex --force && mix local.rebar --force && mix deps.get && mix compile';
  const buildCmd   = opts.project.buildCmd   || 'mix assets.deploy && mix ecto.migrate';
  const startCmd   = opts.project.startCmd   || `mix phx.server`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'elixir',
...
async function runBunBuild(opts) {
  ...
  const port    = normalizePort(opts.appPort, 3000);
  const installCmd = opts.project.installCmd || 'bun install --frozen-lockfile';
  const buildCmd   = opts.project.buildCmd   || 'bun run build';
  const startCmd   = opts.project.startCmd   || `PORT=${port} bun run start`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'bun',
...
async function runDenoBuild(opts) {
  ...
  const installCmd = opts.project.installCmd || 'echo skip';
  const buildCmd   = opts.project.buildCmd   || 'echo skip';
  const startCmd   = opts.project.startCmd   || `PORT=${port} deno task start`;
  const p = { ...opts.project, installCmd, buildCmd, startCmd };
  return runGenericBuild({ ...opts, project: p, dockerImage: image, stepLabel: 'deno',
