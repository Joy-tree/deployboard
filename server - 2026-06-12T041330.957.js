...
//   · a plain number / numeric string (legacy): 10018
//   · "host:port" proxy format (current): "127.0.0.1:10018"
...
function getOrAssignPort(subdomain) {
  ...
  // Parse the existing entry regardless of format ("127.0.0.1:10018" or 10018)
  const assigned = _portFromEntry(portRegistry[subdomain]);
  // Reuse the existing port for redeploys — even if it's currently "in use"
  // (the old container is about to be paused/stopped by buildRunner, freeing it).
  // Only pick a NEW port if there's no previously assigned port at all.
  if (Number.isFinite(assigned) && assigned >= PORT_START && assigned <= PORT_END) {
    return assigned;
  }
  ...
  for (let p = PORT_START; p <= PORT_END; p++) {
    if (!used.has(p) && isPortAvailable(p)) {
      // Store as plain number for now — buildRunner will overwrite with the
      // "127.0.0.1:port" proxy format once the container is live.
