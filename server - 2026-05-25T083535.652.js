...
async function buildFlowWithGemini({ prompt = '', sourceText = '', fileBase64 = '', fileMime = '', fileName = '' } = {}) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      console.error('[Gemini] HTTP error', r.status, errText.slice(0, 300));
      return null;
    }
...
async function buildFlowWithGroq({ prompt = '', sourceText = '', fileName = '' } = {}) {
  ...
  try {
    ...
    if (!r.ok) {
      const errText = await r.text().catch(() => '');
      console.error('[Groq] HTTP error', r.status, errText.slice(0, 300));
      return null;
    }
    const data = await r.json().catch(() => ({}));
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  const aiSpec = await buildFlowWithAI({ prompt, sourceText: `${sourceText}\n${scrapedContext}`, fileBase64, fileMime, fileName, aiProvider: providerRequested }).catch(() => null);
  if (!aiSpec) return res.status(502).json({ error: 'AI did not return a valid flow spec. Try refining your prompt or check your API key.' });
  if (!aiSpec.responseTemplate || typeof aiSpec.responseTemplate !== 'object') return res.status(502).json({ error: 'AI returned invalid API output template.' });
  ...
  // Clean internal keys before using spec
  ...
  if (isQuiz) {
    ...
    if (!questions.length) return res.status(502).json({ error: 'AI returned no quiz/riddle content. Refine your prompt (e.g. specify question count + format).' });
