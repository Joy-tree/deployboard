...
async function runUploadStaticBuild({ deployId, project, sitesDir, tmpDir, emit, onLog, uploadFilesDir, log, cleanSub }) {
  ...
  if (hasPackageJson) {
    ...
    log('\x1b[90m[install] No package.json found — skipping install for plain static files\x1b[0m');
    // Also force outputDir to '.' so we serve the project root directly
    if (!project.outputDir || project.outputDir === 'dist') {
      log('\x1b[33m[Joytree]\x1b[0m Output directory automatically changed from "' + (project.outputDir || 'dist') + '" → "." (no package.json, serving project root)');
      project = { ...project, outputDir: '.' };
    }
  }
  emitStep(emit, 'install', 'done');
  ...
  if (buildCmd === 'echo skip') {
    ...
    try {
      ...
      if (/missing script|npm ERR!.*build|yarn.*command not found.*build|pnpm.*command not found.*build/i.test(String(e.message || ''))) {
        ...
        // If outputDir was pointing to a build artifact dir that won't exist, fall back to '.'
        if (!project.outputDir || project.outputDir === 'dist') {
          log(`\x1b[33m[Joytree]\x1b[0m Output directory automatically changed from "${project.outputDir || 'dist'}" → "." (serving project root since no build was run)`);
          project = { ...project, outputDir: '.' };
        }
      } else {
        throw e;
  ...
  const srcDist = path.join(projectRoot, outputDir);
  const destDist = path.join(sitesDir, cleanSub, 'dist');
  // Always clear destDist before copying so stale files from a previous deploy
  // never mask the new index.html or cause "Not found" after redeployment.
