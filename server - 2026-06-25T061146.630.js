...
// Maps subdomain → externalUrl for projects that mirror an external site
  ...
  if (externalUrl) {
    try {
      ...
      const extReq = lib.request(opts, (extRes) => {
        ...
        // Rewrite absolute redirects pointing to the external origin back to the JoyTree subdomain
      ...
      // Fall through to normal routing if external proxy fails
    }
  }
...
app.post('/api/domains/proxy-import', async (req, res) => {
  try {
    ...
    addActivity('domain', `External proxy set: ${subdomain} → ${externalUrl}`);
...
app.delete('/api/domains/proxy-import/:subdomain', async (req, res) => {
  try {
    ...
    addActivity('domain', `External proxy removed: ${subdomain}`);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        const dbSubdomain = publicDbHost({ ...newDbRec, id: String(dbRecord._id) }).split('.')[0];
  ...
  try {
    ...
    switch (db.engine) {
      ...
      case 'mariadb': {
        ...
        // Write a credentials .cnf file to avoid quoting issues with passwords inside sh -c '...'
        const myCnfContent = `[client]\nuser=${db.user||'root'}\npassword=${db.pass||''}\n`;
        const cnfFile = tmpFile.replace('.sql', '.cnf');
        fs.writeFileSync(cnfFile, myCnfContent, 'utf8');
        try {
          // Copy the .sql file and .cnf file into the container's /tmp
          const copyCmd = `docker cp ${tmpFile} ${db.containerName}:/tmp/jt_query.sql && docker cp ${cnfFile} ${db.containerName}:/tmp/jt_query.cnf`;
          const copyResult = await runDocker(copyCmd, 10000);
          if (!copyResult.ok) {
            fs.unlinkSync(tmpFile);
            try { fs.unlinkSync(cnfFile); } catch (_) {}
          ...
          // Use --defaults-extra-file so credentials never touch the shell command line
          cmd = `docker exec ${db.containerName} sh -c '${mysqlBin} --defaults-extra-file=/tmp/jt_query.cnf ${db.dbName||'mydb'} --batch --silent < /tmp/jt_query.sql && rm -f /tmp/jt_query.sql /tmp/jt_query.cnf' 2>&1`;
        } finally {
          // Always remove the host tmpfiles
