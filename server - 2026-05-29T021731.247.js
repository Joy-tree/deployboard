...
const INTER_CHUNK_DELAY_MS = 120000; // 2 minutes
...
async function buildQuestionsInChunks({ prompt = '', total = 0, aiVersion = 'v1', socketId = null } = {}) {
  ...
  for (let chunk = 0; chunk < totalChunks; chunk++) {
    ...
    // v3 = Cerebras; v2 = Groq then xAI fallback; v1 = Groq only
    const providers = aiVersion === 'v3'
      ? ['cerebras']
    ...
    for (let attempt = 0; attempt < CHUNK_MAX_RETRIES; attempt++) {
      ...
      for (let pi = 0; pi < providers.length; pi++) {
        ...
        try {
          emitProgress(allQuestions.length, chunk + 1, 'Generating batch ' + (chunk + 1) + '/' + totalChunks + ' via ' + provider + ' \u2014 ' + allQuestions.length + '/' + total + ' done...');
...
app.post('/api/developer/flows/from-text', requireAuth, (req, res) => {
  ...
  const routeTimeoutMs = estimatedChunks > 1
    ? Math.min(10800000, estimatedChunks * 210000 + 60000)
    : 300000;
